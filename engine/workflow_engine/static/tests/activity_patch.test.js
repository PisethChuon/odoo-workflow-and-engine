...
import { defineApprovalsModels } from "@workflow_engine/../tests/approvals_test_helpers";
...
test("activity with approval to be made by logged user", async () => {
    ...
    const requestId = pyEnv["workflow.approval.request"].create({});
    pyEnv["workflow.approval.approver"].create({
    ...
    pyEnv["mail.activity"].create({
        can_write: true,
        res_id: requestId,
        res_model: "workflow.approval.request",
        user_id: serverState.userId,
    });
    await start();
    await openFormView("workflow.approval.request", requestId);
...
test("approve approval", async () => {
    ...
    onRpc("workflow.approval.approver", "action_approve", (args) => {
...
test("refuse approval", async () => {
    ...
    const def = new Deferred();
    onRpc("workflow.approval.approver", "action_refuse", (args) => {
        expect(args.args.length).toBe(1);
        expect(args.args[0]).toBe(requestId);
        step("action_refuse");
        def.resolve();
