...
class ApprovalCategoryApprover(models.Model):
    """ Intermediate model between workflow.approval.category and res.users
        To know whether an approver for this category is required or not
    """
    _name = 'workflow.approval.category.approver'
    _description = 'Approval Type Approver'
    _rec_name = 'user_id'
    _order = 'sequence'

    sequence = fields.Integer('Sequence', default=10)
    category_id = fields.Many2one('workflow.approval.category', string='Approval Type', ondelete='cascade',
                                  required=True)
