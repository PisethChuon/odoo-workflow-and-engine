...
from odoo.addons.workflow_studio.controllers.report import get_report_view_copy
...
def _in_workflow_studio(context):
    return context.get("workflow_studio")
...
class Base(models.AbstractModel):
    ...
    def _get_view_cache_key(self, *args, **kwargs):
        if _in_workflow_studio(self.env.context):
            self = self.with_context(no_address_format=True)
        key = super()._get_view_cache_key(*args, **kwargs)
        return key + (bool(_in_workflow_studio(self.env.context)),)
    ...
    def get_views(self, views, options=None):
        ...
        """
        if options and options.get("workflow_studio"):
            self = self.with_context(workflow_studio=True, no_address_format=True)
...
class IrUiView(models.Model):
    _name = 'ir.ui.view'
    _description = 'View'
    _inherit = ['workflow.studio.mixin', 'ir.ui.view']
    ...
    def _get_x2many_missing_view_archs(self, field, field_node, node_info):
        ...
        if not missing or not _in_workflow_studio(self.env.context):
            return missing
    ...
    def set_studio_map_popup_fields(self, model, node):
        ...
        if field_ids:
            node.attrib['workflow_studio_map_field_ids'] = json.dumps(field_ids)
    ...
    @api.model
    def _workflow_studio_supported_view_types(self):
        type_info = self.fields_get(["type"]).get("type", {})
        return {view_type for view_type, _label in type_info.get("selection", [])}

    @api.model
    def _workflow_studio_view_type_supported(self, view_type):
        return view_type in self._workflow_studio_supported_view_types()
    ...
    def create_automatic_views(self, res_model):
        ...
        supported_view_types = self._workflow_studio_supported_view_types()
        # form, list and search: always
        views |= self.auto_list_view(res_model)

        ir_model = self.env['ir.model']._get(res_model)
        is_approval = bool(getattr(ir_model, "is_approval", False))
        if not is_approval:
        ...
        if 'x_workflow_studio_date' in model._fields:
            views |= self.auto_calendar_view(res_model)
        # gantt: only if x_studio_date_start & x_studio_date_stop
        if 'x_workflow_studio_date_start' in model._fields and 'x_workflow_studio_date_stop' in model._fields:
            if "gantt" in supported_view_types:
                views |= self.auto_gantt_view(res_model)
        # kanban: only if x_studio_stage_id
        if 'x_workflow_studio_stage_id' in model._fields:
            views |= self.auto_kanban_view(res_model)
        # map: only if x_studio_partner_id
        if 'x_workflow_studio_partner_id' in model._fields:
            if "map" in supported_view_types:
                views |= self.auto_map_view(res_model)
        # pivot: only if x_studio_value
        if 'x_workflow_studio_value' in model._fields:
    ...
    def auto_list_view(self, res_model):
        ...
        fields = list()
        if 'x_workflow_studio_sequence' in model._fields:
            fields.append(E.field(name='x_workflow_studio_sequence', widget='handle'))
        fields.append(E.field(name=rec_name))
        if 'x_workflow_studio_partner_id' in model._fields:
            fields.append(E.field(name='x_workflow_studio_partner_id'))
        if 'x_workflow_studio_user_id' in model._fields:
            fields.append(E.field(name='x_workflow_studio_user_id', widget='many2one_avatar_user'))
        if 'x_workflow_studio_company_id' in model._fields:
            fields.append(E.field(name='x_workflow_studio_company_id', groups='base.group_multi_company'))
        if 'x_workflow_studio_currency_id' in model._fields and 'x_workflow_studio_value' in model._fields:
            fields.append(E.field(name='x_workflow_studio_currency_id', invisible='1'))
            fields.append(E.field(name='x_workflow_studio_value', sum=_("Total")))
        if 'x_workflow_studio_tag_ids' in model._fields:
            fields.append(E.field(name='x_workflow_studio_tag_ids', widget='many2many_tags', options="{'color_field': 'x_color'}"))
        if 'x_color' in model._fields:
            fields.append(E.field(name='x_color', widget='color_picker'))
        order = ['id desc']
        if 'x_workflow_studio_sequence' in model._fields:
            order.append('x_workflow_studio_sequence asc')
        if 'x_workflow_studio_priority' in model._fields:
            order.append('x_workflow_studio_priority desc')
    ...
    def auto_form_view(self, res_model):
        ...
        header_content = list()
        if 'x_workflow_studio_stage_id' in model._fields:
            header_content.append(E.field(name='x_workflow_studio_stage_id', widget='statusbar', options="{'clickable': '1'}"))
            sheet_content.append(E.field(name='x_workflow_studio_kanban_state', widget='state_selection'))
        ...
        if 'x_workflow_studio_image' in model._fields:
            sheet_content.append(E.field({'class': 'oe_avatar', 'widget': 'image', 'name': 'x_workflow_studio_image'}))
        ...
        if 'x_workflow_studio_user_id' in model._fields:
            right_group_content.append(E.field(name='x_workflow_studio_user_id', widget='many2one_avatar_user'))
        if 'x_workflow_studio_partner_id' in model._fields:
            left_group_content.append(E.field(name='x_workflow_studio_partner_id'))
            left_group_content.append(E.field(name='x_workflow_studio_partner_phone', widget='phone', options="{'enable_sms': True}"))
            left_group_content.append(E.field(name='x_workflow_studio_partner_email', widget='email'))
        if 'x_workflow_studio_currency_id' in model._fields and 'x_workflow_studio_value' in model._fields:
            right_group_content.append(E.field(name='x_workflow_studio_currency_id', invisible='1'))
            right_group_content.append(E.field(name='x_workflow_studio_value'))
        if 'x_workflow_studio_tag_ids' in model._fields:
            right_group_content.append(E.field(name='x_workflow_studio_tag_ids', widget='many2many_tags', options="{'color_field': 'x_color'}"))
        if 'x_workflow_studio_company_id' in model._fields:
            right_group_content.append(E.field(name='x_workflow_studio_company_id', groups='base.group_multi_company', options="{'no_create': True}"))
        if 'x_workflow_studio_date' in model._fields:
            left_group_content.append(E.field(name='x_workflow_studio_date'))
        if 'x_workflow_studio_date_start' in model._fields and 'x_workflow_studio_date_stop' in model._fields:
            left_group_content.append(
                E.field(name='x_workflow_studio_date_start', string='Dates', widget='daterange', options="{'end_date_field': 'x_workflow_studio_date_stop'}"))
            left_group_content.append(
                E.field(name='x_workflow_studio_date_stop', invisible='1'))
        ...
        if 'x_workflow_studio_notes' in model._fields:
            sheet_content.append(E.group(E.field(name='x_workflow_studio_notes', placeholder=_('Type down your notes here...'), nolabel='1', colspan="2")))
    ...
    def auto_search_view(self, res_model):
        ...
        fields.append(E.field(name=rec_name))
        if 'x_workflow_studio_partner_id' in model._fields:
            fields.append(E.field(name='x_workflow_studio_partner_id', operator='child_of'))
            groupbys.append(E.filter(name='groupby_x_partner', string=_('Partner'), context="{'group_by': 'x_workflow_studio_partner_id'}", domain="[]"))
        if 'x_workflow_studio_user_id' in model._fields:
            fields.append(E.field(name='x_workflow_studio_user_id'))
            filters.append(E.filter(string=_('My %s', model._description), name='my_%s' % res_model, domain="[['x_workflow_studio_user_id', '=', uid]]"))
            groupbys.append(E.filter(name='groupby_x_user', string=_('Responsible'), context="{'group_by': 'x_workflow_studio_user_id'}", domain="[]"))
        date_filters = []
        if 'x_workflow_studio_date' in model._fields:
            date_filters.append(E.filter(date='x_workflow_studio_date', name='studio_filter_date', string=_('Date')))
        if 'x_workflow_studio_date_start' in model._fields and 'x_workflow_studio_date_stop' in model._fields:
            date_filters.append(E.filter(date='x_workflow_studio_date_start', name='workflow_studio_filter_date_start', string=_('Start Date')))
            date_filters.append(E.filter(date='x_workflow_studio_date_stop', name='workflow_studio_filter_date_stop', string=_('End Date')))
        if date_filters:
        ...
        if 'x_active' in model._fields:
            ...
            filters.append(E.separator())
        if 'x_workflow_studio_tag_ids' in model._fields:
            fields.append(E.field(name='x_workflow_studio_tag_ids'))
        if 'x_workflow_studio_stage_id' in model._fields:
            groupbys.append(E.filter(name='x_workflow_studio_stage_id', string=_('Stage'), context="{'group_by': 'x_workflow_studio_stage_id'}", domain="[]"))
        # Invisible activity filters so the activity-tray notification click
        # (search_default_activities_overdue / today / upcoming_all) works for
        # every workflow model — these names must match what activity_menu.js sends.
        if 'my_activity_date_deadline' in model._fields and 'activity_user_id' in model._fields:
            filters.append(E.filter(invisible="1", string="Late Activities",
                                    name="activities_overdue",
                                    domain="['&', ('activity_user_id', '=', uid), ('my_activity_date_deadline', '<', 'today')]"))
            filters.append(E.filter(invisible="1", string="Today Activities",
                                    name="activities_today",
                                    domain="['&', ('activity_user_id', '=', uid), ('my_activity_date_deadline', '=', 'today')]"))
            filters.append(E.filter(invisible="1", string="Future Activities",
    ...
    @api.model
    def _sync_workflow_activity_search_filters(self):
        """Patch the 4 invisible activity filters into every existing auto-generated
        search view for workflow approval models.  Idempotent — skips views that
        already have ``activities_overdue``.  Call once after upgrading the module."""
        approval_models = self.env['ir.model'].sudo().search([
            ('is_approval', '=', True), ('transient', '=', False),
        ])
        activity_filters = [
            ('activities_overdue',      "Late Activities",    "['&', ('activity_user_id', '=', uid), ('my_activity_date_deadline', '<', 'today')]"),
            ('activities_today',        "Today Activities",   "['&', ('activity_user_id', '=', uid), ('my_activity_date_deadline', '=', 'today')]"),
            ('activities_upcoming_all', "Future Activities",  "['&', ('activity_user_id', '=', uid), ('my_activity_date_deadline', '>', 'today')]"),
        ]
        patched = 0
        for ir_model in approval_models:
            views = self.search([('model', '=', ir_model.model), ('type', '=', 'search')])
            for view in views:
                if not view.arch:
                    continue
                root = etree.fromstring(view.arch.encode('utf-8'))
                existing = {f.get('name') for f in root.findall('.//filter')}
                if 'activities_overdue' in existing:
                    continue
                for name, string, domain in activity_filters:
                    root.append(E.filter(invisible="1", string=string, name=name, domain=domain))
                view.sudo().write({'arch': etree.tostring(root, encoding='unicode', pretty_print=True)})
                patched += 1
        return patched

    def auto_calendar_view(self, res_model):
        model = self.env[res_model]
        if not 'x_workflow_studio_date' in model._fields:
            return self
        calendar = E.calendar(date_start='x_workflow_studio_date', create_name_field='x_name')
    ...
    def auto_gantt_view(self, res_model):
        if not self._workflow_studio_view_type_supported("gantt"):
            return self.env['ir.ui.view']
        gantt = E.gantt(date_start='x_workflow_studio_date_start', date_stop='x_workflow_studio_date_stop')
    ...
    def auto_map_view(self, res_model):
        if not self._workflow_studio_view_type_supported("map"):
            return self.env['ir.ui.view']
        field = E.field(name='x_workflow_studio_partner_id', string=_('Partner'))
        map_view = E.map(field, res_partner='x_workflow_studio_partner_id')
    ...
    def auto_pivot_view(self, res_model):
        model = self.env[res_model]
        fields = list()
        fields.append(E.field(name='x_workflow_studio_value', type='measure'))
        if 'x_workflow_studio_stage_id' in model._fields:
            fields.append(E.field(name='x_workflow_studio_stage_id', type='col'))
        if 'x_workflow_studio_date' in model._fields:
            fields.append(E.field(name='x_workflow_studio_date', type='row'))
    ...
    def auto_kanban_view(self, res_model):
        ...
        card_header.append(E.field(name='x_workflow_studio_priority', widget='boolean_favorite', nolabel='1'))
        ...
        if 'x_workflow_studio_sequence' in model._fields:
            card_header.append(E.field(name='x_workflow_studio_sequence', widget='handle', invisible='1'))
        if 'x_workflow_studio_kanban_state' in model._fields:
            status = E.field(name='x_workflow_studio_kanban_state', widget='state_selection')
        ...
        if 'x_workflow_studio_value' and 'x_workflow_studio_currency_id' in model._fields:
            pre_fields.append(E.field(name='x_workflow_studio_currency_id'))
            card_footer.append(E.field(name='x_workflow_studio_value'))
        if 'x_workflow_studio_tag_ids' in model._fields:
            card_body.append(E.field(name='x_workflow_studio_tag_ids', options="{'color_field': 'x_color'}"))
        ...
        order = 'x_workflow_studio_priority desc,x_workflow_studio_sequence asc,id desc' if 'x_workflow_studio_sequence' in model._fields else 'x_workflow_studio_priority desc,id desc'
        kanban = E.kanban(default_group_by='x_workflow_studio_stage_id', default_order=order, highlight_color="x_color")
        kanban.extend(pre_fields)
        if 'x_workflow_studio_value' in model._fields:
            progressbar = E.progressbar(field='x_workflow_studio_kanban_state', colors='{"normal": "200", "done": "success", "blocked": "danger"}', sum_field='x_workflow_studio_value')
        else:
            progressbar = E.progressbar(field='x_workflow_studio_kanban_state', colors='{"normal": "200", "done": "success", "blocked": "danger"}')
    ...
    def is_workflow_studio_view(self):
        return self.xml_id.startswith('workflow_studio_customization')
    ...
    def _groups_branding(self, specs_tree):
        ...
        is_workflow_studio = _in_workflow_studio(self.env.context)
        check_view_ids = self.env.context.get('check_view_ids')
        if group_ids and (not is_workflow_studio or not check_view_ids):
    ...
    def apply_inheritance_specs(self, source, specs_tree, pre_locate=None):
        ...
        if self and self.is_workflow_studio_view():
    ...
    def normalize_with_keyed_tree(self, base_arch, arch_to_normalize) -> str:
        ...
        def on_new_node(node):
            if node.tag in CONTAINER_TYPES and not node.get("name"):
                ...
                node.set('name', 'workflow_studio_%s_%s' % (node.tag, uid))
    ...
    def _validate_tag_button(self, node, name_manager, node_info):
        ...
        workflow_studio_approval = node.get('workflow_studio_approval')
        if workflow_studio_approval and self.type != 'form':
            self._raise_view_error(_("workflow_studio_approval attribute can only be set in form views"), node)
        if workflow_studio_approval and workflow_studio_approval not in ['True', 'False']:
            self._raise_view_error(_("Invalid workflow_studio_approval %s in button", workflow_studio_approval), node)

    def _contains_branded(self, node):
        if not _in_workflow_studio(self.env.context):
...
class ResetViewArchWizard(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if self.env.context.get("workflow_studio_report_diff"):
