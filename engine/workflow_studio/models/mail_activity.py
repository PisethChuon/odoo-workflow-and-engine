...
class MailActivity(models.Model):
    ...
    workflow_studio_approval_request_id = fields.Many2one("workflow.studio.approval.request", index=True, ondelete='cascade')

    def _action_done(self, feedback=False, attachment_ids=False):
        activities = self
        approval_activities = self.filtered("workflow_studio_approval_request_id")
        if approval_activities:
            approval_requests = approval_activities.sudo().workflow_studio_approval_request_id
            domains = []
            pairs = set()
            ...
            extra_requests = self.env["workflow.studio.approval.request"].sudo().search(domain)
