...
class StudioApprovalRuleApprover(models.Model):
    _name = 'workflow.studio.approval.rule.approver'
    # # Keep legacy SQL table names for upgrade compatibility with existing DBs.
    # _table = "studio_approval_rule_approver"
    _description = "Approval Rule Approvers Enriched"
    _order = "id desc"
    _log_access = True  # explicit: we need create_uid and create_date

    user_id = fields.Many2one("res.users", required=True, ondelete="cascade")
    rule_id = fields.Many2one("workflow.studio.approval.rule", required=True, index=True, ondelete="cascade")
    date_to = fields.Date()
...
class StudioApprovalRule(models.Model):
    _name = 'workflow.studio.approval.rule'
    # Keep legacy SQL table names for upgrade compatibility with existing DBs.
    # _table = "studio_approval_rule"
    _description = "Studio Approval Rule"
    _inherit = ["workflow.studio.mixin", 'mail.thread']
    ...
    approver_log_ids = fields.One2many("workflow.studio.approval.rule.approver", inverse_name="rule_id", tracking=True)
    ...
    users_to_notify = fields.Many2many(
        ...
        column1="workflow_studio_approval_rule_id",
    ...
    entry_ids = fields.One2many('workflow.studio.approval.entry', 'rule_id', string='Entries')
        ...
        for base_auto in automations:
            ...
            if server_action:
                ...
                actions_server_data.append({
                    ...
                    "base_automation_id": base_auto.id,
                    'state': 'code',
                    'code': "records.env['workflow.studio.approval.entry']._delete_entries(records)",
                    "model_id": model_id.id,
                })
    ...
    def _register_hook(self):
        ...
        def _make_approval_method(method_name, model_name):
            ...
            def method(self, *args, **kwargs):
                ...
                for record in self:
                    result = self.env['workflow.studio.approval.rule'].check_approval(model_name, record.id, method_name, None)
                ...
                if all(approved):
                    ...
                    if len(self) > 1:
                        ...
                        missing_approvals = self.env['workflow.studio.approval.rule'].get_missing_approvals(rules[0], entries[0])
    ...
    def delete_approval(self, res_id):
        ...
        existing_entry = self.env['workflow.studio.approval.entry'].sudo().search([
        ...
        if existing_entry and existing_entry.user_id != self.env.user:
            rules_above = self.env["workflow.studio.approval.rule"].sudo().search_read([
    ...
    def _set_approval(self, res_id, approved):
        ...
        :rtype: :class:`~odoo.addons.workflow_studio.models.StudioApprovalEntry`
        :raise: odoo.exceptions.AccessError when the user does not have write
                access to the underlying record
        :raise: odoo.exceptions.UserError when any of the other checks failed
        """
        ...
        if not all_rule_ids:
            return None
        # Use the model table name to remain compatible with module table prefixes.
        self.env.cr.execute(f'SELECT id FROM "{self._table}" WHERE id IN %s FOR UPDATE NOWAIT', (all_rule_ids,))
        ...
        existing_entry = ruleSudo.env['workflow.studio.approval.entry'].search([
        ...
        result = ruleSudo.env['workflow.studio.approval.entry'].create({
        ...
        if approved and ruleSudo.notification_order != '9':
            ...
            if same_level_rules:
                approved_entries = ruleSudo.env["workflow.studio.approval.entry"].search_read([
    ...
    def _get_approval_spec(self, model, spec):
        ...
        if rule_ids_for_entries:
            ...
            entries_data = self.env['workflow.studio.approval.entry'].sudo().search_read(
    ...
    def _create_request(self, res_id):
        ...
        requests = self.env['workflow.studio.approval.request'].sudo().search([('rule_id', '=', self.id), ('res_id', '=', res_id)])
        if requests:
        ...
        if self.notification_order != '1':
            ...
            entry_sudo = self.env["workflow.studio.approval.entry"].sudo()
        ...
        self.env['workflow.studio.approval.request'].sudo().create([
            {
                'rule_id': self.id,
    ...
    def _unlink_request(self, res_id):
        self.ensure_one()
        sar = self.env['workflow.studio.approval.request'].search([('rule_id', '=', self.id), ('res_id', '=', res_id)])
        sar.unlink()
        return True
    ...
    def open_delegate_action(self):
        ...
        return {
            ...
            "name": _("Delegate to"),
            "target": "new",
            "res_model": "workflow.studio.approval.rule.delegate",
            "views": [[False, "form"]],
            "context": {
                "active_test": True,
    ...
    def open_kanban_rules(self, model, method=False, action=False):
        ...
        context = {
            ...
            "search_view_ref": "workflow_studio.studio_approval_rule_button_configuration_search_view",
        ...
        return {
            ...
            "res_model": "workflow.studio.approval.rule",
            "type": "ir.actions.act_window",
            "views": [
                [False, "kanban"],
                [False, "list"],
...
class StudioApprovalEntry(models.Model):
    _name = 'workflow.studio.approval.entry'
    # _table = "studio_approval_entry"
    ...
    rule_id = fields.Many2one('workflow.studio.approval.rule', string='Approval Rule', ondelete='cascade',
    ...
    def _delete_entries(self, records):
        ...
        existing_entries = self.sudo().env['workflow.studio.approval.entry'].search([
            ('model', '=', model_name),
    ...
    def _notify_approval(self):
        ...
        for entry in self:
            ...
            record.message_post_with_source(
                'workflow_studio.notify_approval',
...
class StudioApprovalRequest(models.Model):
    _name = 'workflow.studio.approval.request'
    # _table = "studio_approval_request"
    _description = 'Workflow Studio Approval Request'
    ...
    def _inverse_mail_activity_id(self):
        for rec in self:
            rec.mail_activity_id.workflow_studio_approval_request_id = rec.id


class StudioApprovalRuleDelegate(models.TransientModel):
    _name = 'workflow.studio.approval.rule.delegate'
    # _table = "studio_approval_rule_delegate"
    _description = "Approval Rule Delegate"

    approval_rule_id = fields.Many2one("workflow.studio.approval.rule", required=True)
    approver_ids = fields.Many2many(
        "res.users",
        relation="res_users_studio_approval_rule_delegate_rel",
        column1="workflow_studio_approval_rule_delegate_id",
        column2="res_users_id",
        string="Approvers",
    ...
    def default_get(self, fields):
        ...
        if default_rule_id and "approver_ids" in fields and "users_to_notify" in fields:
            rule = self.env["workflow.studio.approval.rule"].browse(default_rule_id).sudo()
