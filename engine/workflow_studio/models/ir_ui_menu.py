...
class IrUiMenu(models.Model):
    _name = 'ir.ui.menu'
    _description = 'Menu'
    _inherit = ['workflow.studio.mixin', 'ir.ui.menu']

    is_workflow_studio_configuration = fields.Boolean(
        string='Workflow Studio Configuration Menu',
        help='Indicates that this menu was created by Workflow Studio to hold configuration sub-menus',
        readonly=True)
    ...
    def write(self, vals):
        ...
        """
        for menu in self:
            in_studio = menu.env.context.get("workflow_studio")
            if in_studio and 'name' in vals and menu.action:
    ...
    def _get_studio_configuration_menu(self):
        ...
        root_xmlids = self.env['ir.model.data'].search_read(
            ...
            fields=['module', 'name', 'workflow_studio']
        )
        # look for a studio config menu in the submenus
        parent_path = '%s/' % root_id
        config_menu = self.with_context(self.env.context).search([
            ('parent_path', 'like', parent_path), ('is_workflow_studio_configuration', '=', True)
        ])
        if not config_menu:
            is_workflow_studio_app = root_xmlids and any(map(lambda xmlid: xmlid['workflow_studio'], root_xmlids))
            menu_name = _('Configuration') if is_workflow_studio_app else _('Custom Configuration')
            config_menu = self.create({
                'name': menu_name,
                'is_workflow_studio_configuration': True,
                'parent_id': root_id,
                'sequence': 1000,
            })
        return config_menu
