...
class StudioExportWizardData(models.TransientModel):
    ...
    """
    _name = 'workflow.studio.export.wizard.data'
    # _table = "studio_export_wizard_data"
    ...
    workflow_studio = fields.Boolean(default=False, readonly=True)
    ...
    def _xmlid_for_export(self):
        self.ensure_one()
        xmlid = self.xmlid.replace("__export__.", "")
...
class StudioExportWizard(models.TransientModel):
    _name = 'workflow.studio.export.wizard'
    # _table = "studio_export_wizard"
    ...
    def _default_studio_export_data(self):
        ...
        return self.env["workflow.studio.export.wizard.data"].create(
            [{"model": d.model, "res_id": d.res_id, "workflow_studio": d.workflow_studio} for d in active_data]
        )

    default_export_data = fields.Many2many(
        "workflow.studio.export.wizard.data",
        default=_default_studio_export_data,
        relation="rel_studio_export_wizard_data",
        column1="workflow_studio_export_wizard_id",
        column2="workflow_studio_export_wizard_data_id",
    )
    ...
    def _compute_additional_models(self):
        ...
        for record in to_update:
            ...
            record.additional_models = self.env["workflow.studio.export.model"].search(domain)
    ...
    def _compute_export_data(self):
        ...
        for rec in to_update:
            ...
            rec.additional_export_data = self.env["workflow.studio.export.wizard.data"].create(
                export_data_vals
            )
