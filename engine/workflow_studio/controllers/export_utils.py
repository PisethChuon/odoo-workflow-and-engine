...
DEFAULT_WORKFLOW_STUDIO_CUSTOMIZATION_MODULE = "workflow_studio_customization"
...
class StudioExportSerializer:
    ...
    def _serialize_record(self, record, fields, default_get_data):
        ...
        context = {}
        if record_data.workflow_studio:
            context.update({'workflow_studio': True})
    ...
    def _serialize_field(self, record, field, default_get_data):
        ...
        if self.has_website and field.name == 'key' and record._name == 'ir.ui.view' and record.website_id:
            value = f"{self.module.name}.{value}"
        ...
        if not value:
            ...
            return E.field(
                name=field.name,
                type="base64",
                file=f"{self.module.name}/{self.utils.get_binary_field_file_name(field, record)}",
            )
...
class StudioExportUtils:
    ...
    def get_module_name(self, record):
        ...
        if xmlid.startswith('base.module_'):
            ...
            return xmlid[12:]
        if not '.' in xmlid:
            return self.module.name or DEFAULT_WORKFLOW_STUDIO_CUSTOMIZATION_MODULE
