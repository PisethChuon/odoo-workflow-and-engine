...
from odoo.addons.workflow_studio.controllers import main
from odoo.addons.workflow_studio.controllers.keyed_xml_differ import KeyedXmlDiffer, DIFF_ATTRIBUTE
...
VIEW_BACKUP_KEY = "workflow_studio.__backup__._{view.id}_._{view.key}_"
...
def _copy_report_view(view):
    ...
    if not copy:
        key = VIEW_BACKUP_KEY.format(view=view)
        copy = view.copy({
            "name": f"workflow_studio_backup__{view.name}",
            "inherit_id": False,
            "mode": "primary",
            "key": key,
            "active": False,
...
STUDIO_VIEW_KEY_TEMPLATE = "workflow_studio.report_editor_customization_full.view._{key}"
STUDIO_VIEW_DIFF_KEY_TEMPLATE = "workflow_studio.report_editor_customization_diff.view._{key}"
...
class WebStudioReportController(main.WebStudioController):
    ...
    @http.route('/workflow_studio/create_new_report', type='jsonrpc', auth='user')
    ...
    def _create_default_report_views(self, layout='web.basic_layout'):
        env = request.env(context={**request.env.context, "workflow_studio": 1})
    ...
    @http.route('/workflow_studio/print_report', type='jsonrpc', auth='user')
    def print_report(self, report_id, record_id):
        report = request.env['ir.actions.report'].with_context(report_pdf_no_attachment=True, discard_logo_check=True, workflow_studio=1)._get_report(report_id)
        return report.report_action(record_id)

    @http.route('/workflow_studio/load_report_editor', type='jsonrpc', auth='user')
    ...
    @http.route('/workflow_studio/get_report_html', type='jsonrpc', auth='user')
    ...
    @http.route('/workflow_studio/get_report_qweb', type='jsonrpc', auth='user')
    ...
    def _get_report_qweb(self, report):
        loaded = {}
        report = report.with_context(workflow_studio=True)
        report_name = report.report_name
        IrQweb = request.env["ir.qweb"].with_context(workflow_studio=True, lang=None)
        ...
        with report.env.registry.cursor() as nfg_cursor:
            ...
            render_context = report_safe_cr._get_rendering_context(report_safe_cr, [0], {"workflow_studio": True})
            render_context['report_type'] = "pdf"
            main_qweb = _guess_qweb_variables(main_qweb, report_safe_cr, render_context)

        html_container = IrQweb._render("web.html_container", {"workflow_studio": True})
    ...
    def _render_report(self, report, record_id):
        return request.env['ir.actions.report'].with_context(workflow_studio=True)._render_qweb_html(report, [record_id] if record_id else [], {"workflow_studio": True})

    @http.route("/workflow_studio/save_report", type="jsonrpc", auth="user")
    def save_report(self, report_id, report_changes=None, html_parts=None, xml_verbatim=None, record_id=None, context=None):
        ...
        IrView = request.env["ir.ui.view"].with_context(workflow_studio=True, no_cow=True, lang=None)
    ...
    @http.route("/workflow_studio/reset_report_archs", type="jsonrpc", auth="user")
