...
from odoo.addons.workflow_studio.controllers.report import WebStudioReportController, get_report_view_copy, _get_and_write_studio_view
...
class TestReportEditor(TransactionCase):
    ...
    def test_copy_inherit_report(self):
        report = self.env['ir.actions.report'].create({
            ...
            'report_name': 'workflow_studio.test_inherit_report_user',
            'model': 'res.users',
        })
        self.env['ir.ui.view'].create({
            'type': 'qweb',
            'name': 'workflow_studio.test_inherit_report_hi',
            'key': 'workflow_studio.test_inherit_report_hi',
            'arch': '''
                <t t-name="workflow_studio.test_inherit_report_hi">
                    hi
                </t>
            ''',
        })
        parent_view = self.env['ir.ui.view'].create({
            'type': 'qweb',
            'name': 'workflow_studio.test_inherit_report_user_parent',
            'key': 'workflow_studio.test_inherit_report_user_parent',
            'arch': '''
                <t t-name="workflow_studio.test_inherit_report_user_parent_view_parent">
                    <t t-call="workflow_studio.test_inherit_report_hi"/>!
                </t>
            ''',
        })
        self.env['ir.ui.view'].create({
            'type': 'qweb',
            'name': 'workflow_studio.test_inherit_report_user',
            'key': 'workflow_studio.test_inherit_report_user',
            'arch': '''
        ...
        copy_report = self.env['ir.actions.report'].search([
            ('report_name', '=', 'workflow_studio.test_inherit_report_user_copy_1'),
        ])
    ...
    def test_duplicate(self):
        ...
        report = self.env['ir.actions.report'].create({
            'name': 'test duplicate',
            'report_name': 'workflow_studio.test_duplicate_foo',
            'model': 'res.users',})

        self.env['ir.ui.view'].create({
            'type': 'qweb',
            'name': 'test_duplicate_foo',
            'key': 'workflow_studio.test_duplicate_foo',
            'arch': "<t t-name='workflow_studio.test_duplicate_foo'>foo</t>",})

        duplicate_domain = [('report_name', '=like', 'workflow_studio.test_duplicate_foo_copy_%')]
    ...
    def test_duplicate_keep_translations(self):
        def create_view(name, **kwargs):
            ...
            name = 'workflow_studio.test_keep_translations_{}'.format(name)
        ...
        report = self.env['ir.actions.report'].create({
            'name': 'test inherit report user',
            'report_name': 'workflow_studio.test_keep_translations_ab',
            'model': 'res.users',
        ...
        report.copy_report_and_template()
        copy_view = self.env['ir.ui.view'].search([
            ('key', '=', 'workflow_studio.test_keep_translations_ab_copy_1'),
        ])
...
@unittest.skip("Legacy report-editor tours are incompatible with workflow-only Studio mode.")
class TestReportEditorUIUnit(HttpCase):
    ...
    def setUpClass(cls):
        ...
        cls.report = cls.env['ir.actions.report'].create({
            'name': 'test report',
            'report_name': 'workflow_studio.test_report',
            'model': 'res.partner',
        })
        ...
        cls.main_view = cls.env['ir.ui.view'].create({
            'type': 'qweb',
            'name': 'workflow_studio.test_report',
            'key': 'workflow_studio.test_report',
            'arch': '''
                <t t-name="workflow_studio.test_report">
                    <t t-call="web.html_container">
                        <div><p><br/></p></div>
                        <t t-foreach="docs" t-as="doc">
                            <t t-call="workflow_studio.test_report_document" />
                        </t>
                    </t>
                </t>
            ''',
        ...
        cls.main_view_document = cls.env['ir.ui.view'].create({
            'type': 'qweb',
            'name': 'workflow_studio.test_report_document',
            'key': 'workflow_studio.test_report_document',
            'arch': '''
                <t t-name="workflow_studio.test_report_document">
    ...
    @property
    def tour_url(self):
        return f"/odoo/action-{self.testAction.id}/workflow_studio?mode=editor&_tab=reports&_report_id={self.report.id}&menu_id={self.testMenu.id}"
    ...
    def test_basic_report_edition(self):
        ...
        self.start_tour(self.tour_url, "workflow_studio.test_basic_report_edition", login="admin")
        ...
        <data>
           <data>
            <xpath expr="/t[@t-name='workflow_studio.test_report']//t[@t-call='web.html_container']/div/p/br" position="replace"/>
             <xpath expr="/t[@t-name='workflow_studio.test_report']//t[@t-call='web.html_container']/div/p" position="inside">edited with odoo editor</xpath>
           </data>
        </data>
        """)
        ...
        <data>
           <data>
             <xpath expr="/t[@t-name='workflow_studio.test_report_document']/p/br" position="replace"/>
             <xpath expr="/t[@t-name='workflow_studio.test_report_document']/p" position="inside">edited with odoo editor 2</xpath>
           </data>
        </data>
        """)
        ...
        self.assertEqual(
            copied_view.name,
            "workflow_studio_backup__workflow_studio.test_report_document",
        )
        self.assertEqual(copied_view.key, f"workflow_studio.__backup__._{self.main_view_document.id}_._{self.main_view_document.key}_")
    ...
    def test_basic_report_edition_without_datas(self):
        ...
        def _get_rendering_context_mock(self_model, report, docids, data):
            ...
            self.assertTrue(self_model.env.context.get("workflow_studio"))
            self.assertTrue(data.get("workflow_studio"))
    ...
    def test_basic_report_edition_xml(self):
        self.start_tour(self.tour_url, "workflow_studio.test_basic_report_edition_xml", login="admin")
    ...
    def test_basic_report_edition_discard(self):
        self.start_tour(self.tour_url, "workflow_studio.test_basic_report_edition_discard", login="admin")
    ...
    def test_basic_report_edition_cancel_discard(self):
        self.start_tour(self.tour_url, "workflow_studio.test_basic_report_edition_cancel_discard", login="admin")
    ...
    def test_basic_report_edition_xml_discard(self):
        self.start_tour(self.tour_url, "workflow_studio.test_basic_report_edition_xml_discard", login="admin")
    ...
    def test_basic_report_edition_error(self):
        ...
        @route('/workflow_studio/save_report', type='jsonrpc', auth='user')
        ...
        with mute_logger("odoo.http"):
            self.start_tour(self.tour_url, "workflow_studio.test_basic_report_edition_error", login="admin")
    ...
    def test_basic_report_edition_xml_error(self):
        ...
        with mute_logger("odoo.http"):
            self.start_tour(self.tour_url, "workflow_studio.test_basic_report_edition_xml_error", login="admin")
    ...
    def test_report_reset_archs(self):
        self.main_view_document.arch_fs = "workflow_studio/tests/test_report_editor.xml"
        self.start_tour(self.tour_url, "workflow_studio.test_report_reset_archs", login="admin")
    ...
    def test_report_reset_archs_studio_view(self):
        ...
        self.url_open("/workflow_studio/reset_report_archs",
    ...
    def test_print_preview(self):
        self.start_tour(self.tour_url, "workflow_studio.test_print_preview", login="admin")

    def test_reports_view(self):
        self.start_tour(self.tour_url, "workflow_studio.test_reports_view_concurrence", login="admin")
    ...
    def test_table_rendering(self):
        ...
        self.start_tour(self.tour_url, "workflow_studio.test_table_rendering", login="admin")
    ...
    def test_nested_table_rendering(self):
        ...
        response = self.url_open(
            "/workflow_studio/get_report_qweb",
            data=json.dumps({"params": {"report_id": self.report.id}}),
            headers={"Content-Type": "application/json"}
        )
        ...
        div_node = tree.xpath("//t[@t-name='workflow_studio.test_report_document']")[0][0]
    ...
    def test_field_placeholder(self):
        ...
        self.start_tour(self.tour_url + "&debug=assets", "workflow_studio.test_field_placeholder", login="admin")
    ...
    def test_toolbar_appearance(self):
        ...
        self.start_tour(self.tour_url, "workflow_studio.test_toolbar_appearance", login="admin")

    def test_add_field_blank_report(self):
        studio_views = self.env["ir.ui.view"].search([("key", "=ilike", "workflow_studio_customization.%")])
        self.start_tour(self.tour_url, "workflow_studio.test_add_field_blank_report", login="admin")
        new_view = self.env["ir.ui.view"].search([("key", "=ilike", "workflow_studio_customization.%"), ("id", "not in", studio_views.ids), ("name", "like", "document")])
    ...
    def test_edition_without_lang(self):
        ...
        self.start_tour(self.tour_url, "workflow_studio.test_edition_without_lang", login="admin")
        ...
        <data>
            <data>
                 <xpath expr="/t[@t-name='workflow_studio.test_report_document']/p" position="replace">
    ...
    def test_report_xml_other_record(self):
        ...
        self.start_tour(self.tour_url, "workflow_studio.test_report_xml_other_record", login="admin")
    ...
    def test_partial_eval(self):
        ...
        """
        self.start_tour(self.tour_url, "workflow_studio.test_partial_eval", login="admin")
    ...
    def test_render_multicompany(self):
        ...
        external_report_layout_id = self.env['ir.ui.view'].create({
            'type': 'qweb',
            'name': 'workflow_studio.test_layout',
            'key': 'workflow_studio.test_layout',
            'arch': '''
                <t t-name="workflow_studio.test_report">
        ...
        self.start_tour(self.tour_url, "workflow_studio.test_render_multicompany", login="admin", cookies={"cids": f"{company2.id}-{company1.id}"})
    ...
    def test_report_edition_binary_field(self):
        ...
        self.start_tour(self.tour_url, "workflow_studio.test_report_edition_binary_field", login="admin")
    ...
    def test_report_edition_dynamic_table(self):
        self.start_tour(self.tour_url + "&debug=assets", "workflow_studio.test_report_edition_dynamic_table", login="admin")

        arch, _ = get_combined_and_studio_arch(self.main_view_document)
        arch_tree = etree.fromstring(arch)
        self.assertEqual(len(arch_tree.xpath(".//table[@class='table table-sm']")), 1)
        self.assertEqual(
            len(arch_tree.xpath(".//tr[@t-foreach='doc.activity_ids'][@t-as='x2many_record']")),
            1,
        )
        self.assertEqual(
            len(
                arch_tree.xpath(
                    ...
                    "//span[@t-field='x2many_record.summary']"
                )
            ),
            1,
        )
    ...
    def test_saving_xml_editor_reload(self):
        self.start_tour(self.tour_url, "workflow_studio.test_saving_xml_editor_reload", login="admin")
    ...
    def test_error_at_loading(self):
        ...
        with mute_logger("odoo.http"):
            self.start_tour(self.tour_url, "workflow_studio.test_error_at_loading", login="admin")

        with mute_logger("odoo.http"):
            self.start_tour(self.tour_url + "&debug=assets", "workflow_studio.test_error_at_loading_debug", login="admin")

    def test_xml_and_form_diff(self):
        self.start_tour(self.tour_url + "&debug=1", "workflow_studio.test_xml_and_form_diff", login="admin")
    ...
    def test_recursive_t_calls(self):
        ...
        self.main_view_document.arch = """
             <t t-name="workflow_studio.test_report_document">
                <div><p t-field="doc.name" /></div>
                <p><br/></p>
                <t t-call="workflow_studio.test_report_document"><div /></t>
            </t>
        """
        ...
        tree = html_to_xml_tree(qweb_html)
        _clean_for_simple_assert(tree)
        tcall = tree.xpath("//t[@t-call='workflow_studio.test_report_document']")[0]
        self.assertXMLEqual(etree.tostring(tcall), f"""
           <t t-call="workflow_studio.test_report_document" ws-call-key="2" ws-view-id="{self.main_view.id}">
             <t t-name="workflow_studio.test_report_document" ws-view-id="{self.main_view_document.id}">
               <div>
               ...
               <t t-call="workflow_studio.test_report_document" ws-call-key="1" ws-view-id="{self.main_view_document.id}">
                    <div/>
               </t>
             </t>
           </t>
    ...
    def test_can_have_multiple_times_same_t_call(self):
        ...
        """
        self.main_view_document.arch = """<t t-name="workflow_studio.test_report_document"><t t-out="0" /></t> """
        response = self.url_open(
            "/workflow_studio/get_report_qweb",
            data=json.dumps({"params": {"report_id": self.report.id}}),
            headers={"Content-Type": "application/json"}
        ...
        tree = html_to_xml_tree(qweb_html)
        _clean_for_simple_assert(tree)
        tcall = tree.xpath("//t[@t-call='workflow_studio.test_report_document']")
        ...
        <t t-call="workflow_studio.test_report_document" ws-call-key="3" ws-view-id="{self.main_view.id}">
           <t t-name="workflow_studio.test_report_document" ws-view-id="{self.main_view_document.id}">
             <t oe-origin-t-out="0">
    ...
    def test_remove_branding_on_copy(self):
        self.start_tour(self.tour_url, "workflow_studio.test_remove_branding_on_copy", login="admin")

    def test_test_different_view_document_name(self):
        self.main_view_document_xml_id.name = "test_report_document_1"
        self.start_tour(self.tour_url, "workflow_studio.test_different_view_document_name", login="admin")
    ...
    def test_formalize_qweb_client_t_call_structure(self):
        ...
        tcalled_0 = self.env["ir.ui.view"].create({
            'type': 'qweb',
            'name': 'workflow_studio.t_call_0',
            'key': 'workflow_studio.t_call_0',
            'arch': """<t t-name="workflow_studio.t_call_0">
        ...
        tcalled_1 = self.env["ir.ui.view"].create({
            'type': 'qweb',
            'name': 'workflow_studio.t_call_1',
            'key': 'workflow_studio.t_call_1',
            'arch': """<t t-name="workflow_studio.t_call_1">
        ...
        self.main_view_document.arch = """
                <t t-name="workflow_studio.test_report_document">
                    <div>
                        <t t-call="workflow_studio.t_call_0">
                            <div>
                                <p> call zero one </p>
                                <t t-call="workflow_studio.t_call_1">
                                    <p> call zero two </p>
                                </t>
                            </div>
                        </t>
                 ...
                 <t ws-was-t-out="1">
                   <t ws-view-id="{main_view.id}" ws-call-group-key="1" ws-call-key="1" ws-real-children="1">
                     ...
                     <t t-foreach="docs" t-as="doc" oe-context="{&quot;docs&quot;: {&quot;model&quot;: &quot;res.partner&quot;, &quot;name&quot;: &quot;Contact&quot;, &quot;in_foreach&quot;: false}, &quot;company&quot;: {&quot;model&quot;: &quot;res.company&quot;, &quot;name&quot;: &quot;Companies&quot;, &quot;in_foreach&quot;: false}, &quot;doc&quot;: {&quot;model&quot;: &quot;res.partner&quot;, &quot;name&quot;: &quot;Contact&quot;, &quot;in_foreach&quot;: true}}">
                       <t t-call="workflow_studio.test_report_document" ws-call-key="2" ws-view-id="{main_view.id}">
                         <t t-name="workflow_studio.test_report_document" ws-view-id="{main_view_document.id}" oe-context="{&quot;docs&quot;: {&quot;model&quot;: &quot;res.partner&quot;, &quot;name&quot;: &quot;Contact&quot;, &quot;in_foreach&quot;: false}, &quot;company&quot;: {&quot;model&quot;: &quot;res.company&quot;, &quot;name&quot;: &quot;Companies&quot;, &quot;in_foreach&quot;: false}, &quot;doc&quot;: {&quot;model&quot;: &quot;res.partner&quot;, &quot;name&quot;: &quot;Contact&quot;, &quot;in_foreach&quot;: true}}">
                           <div>
                             <t t-call="workflow_studio.t_call_0" ws-call-key="1" ws-view-id="{main_view_document.id}">
                               <t t-name="workflow_studio.t_call_0" ws-view-id="{tcalled_0.id}" oe-context="{&quot;docs&quot;: {&quot;model&quot;: &quot;res.partner&quot;, &quot;name&quot;: &quot;Contact&quot;, &quot;in_foreach&quot;: false}, &quot;company&quot;: {&quot;model&quot;: &quot;res.company&quot;, &quot;name&quot;: &quot;Companies&quot;, &quot;in_foreach&quot;: false}, &quot;doc&quot;: {&quot;model&quot;: &quot;res.partner&quot;, &quot;name&quot;: &quot;Contact&quot;, &quot;in_foreach&quot;: true}}">
                                 <div class="call_0">
                                   <t ws-was-t-out="1">
                                     <t ws-view-id="{main_view_document.id}" ws-call-group-key="1" ws-call-key="1" ws-real-children="1">
                                       <div>
                                         <p> call zero one </p>
                                         <t t-call="workflow_studio.t_call_1" ws-call-key="2" ws-view-id="{main_view_document.id}">
                                           <t t-name="workflow_studio.t_call_1" ws-view-id="{tcalled_1.id}" oe-context="{&quot;docs&quot;: {&quot;model&quot;: &quot;res.partner&quot;, &quot;name&quot;: &quot;Contact&quot;, &quot;in_foreach&quot;: false}, &quot;company&quot;: {&quot;model&quot;: &quot;res.company&quot;, &quot;name&quot;: &quot;Companies&quot;, &quot;in_foreach&quot;: false}, &quot;doc&quot;: {&quot;model&quot;: &quot;res.partner&quot;, &quot;name&quot;: &quot;Contact&quot;, &quot;in_foreach&quot;: true}}">
                                             <div class="call_1">
                                               <p> call one </p>
                                               <t ws-was-t-out="1">
    ...
    def test_edit_main_arch(self):
        ...
        self.start_tour(self.tour_url, "workflow_studio.test_edit_main_arch", login="admin")
        ...
        <data>
            <data>
                <xpath expr="/t[@t-name='workflow_studio.test_report_document']/div" position="before" meta-class="outside-t-call">
                <div class="added"><br/></div>
                </xpath>
            </data>
         </data>
    ...
    def test_edit_in_t_call(self):
        ...
        self.start_tour(self.tour_url, "workflow_studio.test_edit_in_t_call", login="admin")
        ...
        <data>
            <data>
                <xpath expr="/t[@t-name='workflow_studio.test_report_document']//t[@t-call='workflow_studio.t_call_0']//t[@t-set='somevar']" position="after" meta-t-set="somevar" meta-t-value="'someValue'">
                <div class="added"><br/></div>
                </xpath>
            </data>
        </data>
    ...
    def test_edit_main_and_in_t_call(self):
        ...
        self.start_tour(self.tour_url, "workflow_studio.test_edit_main_and_in_t_call", login="admin")
    ...
    def test_image_crop(self):
        ...
        self.start_tour(self.tour_url, "workflow_studio.test_image_crop", login="admin")
    ...
    def test_add_non_searchable_field(self):
        ...
        self.start_tour(self.tour_url, "workflow_studio.test_add_non_searchable_field", login="admin")
        arch, _ = get_combined_and_studio_arch(self.main_view_document)
        self.assertXMLEqual(arch, """
            <t t-name="workflow_studio.test_report_document">
                <div><p t-field="doc.name"/></div>
    ...
    def test_translations_are_copied(self):
        ...
        self.start_tour(self.tour_url, "workflow_studio.test_translations_are_copied", login="admin")
    ...
    def test_dont_translate_on_save(self):
        ...
        self.env["res.lang"]._activate_lang("fr_FR")
        self.start_tour(self.tour_url, "workflow_studio.test_dont_translate_on_save", login="admin")
        html, _ = self.report.with_context(lang="fr_FR")._render_qweb_html(self.report.id, [1])
        main = etree.fromstring(html).find(".//main")
        self.assertXMLEqual(etree.tostring(main), """
        <main>
    ...
    def test_do_not_delete_unspecial_spans(self):
        ...
        self.start_tour(self.tour_url, "workflow_studio.test_do_not_delete_unspecial_spans", login="admin")
        _, studio_view_arch = get_combined_and_studio_arch(self.main_view_document)
        self.assertXMLEqual(studio_view_arch, """
         <data>
           <data>
           <xpath expr="/t[@t-name='workflow_studio.test_report_document']/span" position="replace">
    ...
    def test_edit_header_only_company(self):
        ...
        self.start_tour(self.tour_url, "workflow_studio.test_edit_header_only_company", login="admin")
        combined = external_layout._get_combined_arch()
        added_fields = combined.xpath(
            "//span[@t-field and contains(@data-oe-demo, 'company id')]"
        )
        self.assertTrue(added_fields)
        added_field = added_fields[0]
        self.assertTrue(added_field.get("t-field").startswith("company."))
        self.assertIn("company id", added_field.get("data-oe-demo"))

    def test_report_without_view(self):
        report = self.env["ir.actions.report"].create({
                "name": "Test Report(No view)",
                "model": "res.partner",
                "report_name": "workflow_studio.test_no_view_report",
                "report_type": "qweb-pdf",
        })

        # Assert that the report has no associated views
        views = self.env["ir.ui.view"].search([("key", "=", "workflow_studio.test_no_view_report")])
        self.assertFalse(views, "The report should not have any associated views.")

        self.start_tour(self.tour_url, "workflow_studio.test_report_without_view", login="admin")
        self.assertNotEqual(report.report_name, "workflow_studio.test_no_view_report")
