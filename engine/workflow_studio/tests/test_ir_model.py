...
from odoo.addons.workflow_studio.wizard.studio_export_wizard import DEFAULT_MODELS_TO_EXPORT, FIELDS_TO_EXPORT, MODELS_WITH_NOUPDATE, RELATIONS_NOT_TO_EXPORT
from odoo.addons.workflow_studio.models.studio_export_model import PRESET_MODELS_DEFAULTS, \
    DEFAULT_FIELDS_TO_EXCLUDE, ABSTRACT_MODEL_FIELDS_TO_EXCLUDE, RELATED_MODELS_TO_EXCLUDE
from odoo.addons.workflow_studio.controllers.export_utils import XML_FIELDS
from odoo.addons.workflow_studio.models.ir_model import OPTIONS_WL
...
class TestStudioIrModel(TransactionCase):
    ...
    def test_00_model_creation(self):
        ...
        expected_fields = ['x_workflow_studio_partner_id', 'x_workflow_studio_stage_id', 'x_workflow_studio_image',
                           'x_workflow_studio_user_id', model.model + '_line_ids']
    ...
    def test_01_mail_inheritance(self):
        ...
        bfr = self.env[model.model].create({
            'x_workflow_studio_partner_id': self.partner_elon.id,
        })
        ...
        self.assertIn('x_workflow_studio_partner_id', sms_suggested_recipients,
    ...
    def test_03_model_option_sequence(self):
        ...
        self.assertEqual(len(extra_models), 0, 'no extra model should have been created')
        fields = self.env[model.model]._fields
        self.assertIn('x_workflow_studio_sequence', fields, 'a custom sequence field should be set up')
        default = self.env['ir.default']._get(model.model, 'x_workflow_studio_sequence')
        self.assertEqual(default, 10, 'the default value for the x_workflow_studio_sequence field should be 10')
        ...
        self.assertEqual(tree.attrib['default_order'], 'x_workflow_studio_sequence asc,id desc')
    ...
    def test_04_model_option_responsible(self):
        ...
        self.assertEqual(len(extra_models), 0, 'no extra model should have been created')
        fields = self.env[model.model]._fields
        self.assertIn('x_workflow_studio_user_id', fields, 'a custom responsible (res.users) field should be set up')
        resp_field = self.env['ir.model.fields'].search([('name', '=', 'x_workflow_studio_user_id'), ('model_id', '=', model.id)])
        self.assertTrue(resp_field.tracking, 'the x_workflow_studio_user_id field should be tracked')
    ...
    def test_05_model_option_partner(self):
        ...
        self.assertEqual(len(extra_models), 0, 'no extra model should have been created')
        fields = self.env[model.model]._fields
        self.assertIn('x_workflow_studio_partner_id', fields, 'a custom partner field should be set up')
        self.assertIn('x_workflow_studio_partner_phone', fields, 'a related field x_workflow_studio_partner_phone should be set up')
        self.assertIn('x_workflow_studio_partner_email', fields, 'a related field x_workflow_studio_partner_email should be set up')
        partner_field = self.env['ir.model.fields'].search([('name', '=', 'x_workflow_studio_partner_id'), ('model_id', '=', model.id)])
        self.assertTrue(partner_field.tracking, 'the x_workflow_studio_partner_id field should be tracked')
    ...
    def test_06_model_option_company(self):
        ...
        self.assertEqual(len(extra_models), 0, 'no extra model should have been created')
        fields = self.env[model.model]._fields
        self.assertIn('x_workflow_studio_company_id', fields, 'a custom company field should be set up')
        mc_rule = self.env['ir.rule'].search([
            ('model_id', '=', model.id),
            ('domain_force', 'like', 'x_workflow_studio_company_id')
        ])
        self.assertEqual(len(mc_rule), 1, 'there should be a multi-company rule for the model')
        comp_field = self.env['ir.model.fields'].search([('name', '=', 'x_workflow_studio_company_id'), ('model_id', '=', model.id)])
        self.assertTrue(comp_field.tracking, 'the x_workflow_studio_company_id field should be tracked')
        main_company = self.env.ref('base.main_company')
        default = self.env['ir.default']._get(model.model, 'x_workflow_studio_company_id', company_id=main_company.id)
        self.assertEqual(default, main_company.id, 'the default value for the x_workflow_studio_company_id should be set')
        new_company = self.env['res.company'].create({'name': 'SpaceY'})
        new_default = self.env['ir.default']._get(model.model, 'x_workflow_studio_company_id', company_id=new_company.id)
    ...
    def test_07_model_option_notes(self):
        ...
        self.assertEqual(len(extra_models), 0, 'no extra model should have been created')
        fields = self.env[model.model]._fields
        self.assertIn('x_workflow_studio_notes', fields, 'a custom notes field should be set up')
    ...
    def test_08_model_option_date(self):
        ...
        self.assertEqual(len(extra_models), 0, 'no extra model should have been created')
        fields = self.env[model.model]._fields
        self.assertIn('x_workflow_studio_date', fields, 'a custom date field should be set up')
        date_field = self.env['ir.model.fields'].search([('name', '=', 'x_workflow_studio_date'), ('model_id', '=', model.id)])
        self.assertFalse(date_field.tracking, 'the x_workflow_studio_date field should not be tracked')
    ...
    def test_09_model_option_double_dates(self):
        ...
        self.assertEqual(len(extra_models), 0, 'no extra model should have been created')
        fields = self.env[model.model]._fields
        self.assertIn('x_workflow_studio_date_start', fields, 'a custom start date field should be set up')
        self.assertIn('x_workflow_studio_date_stop', fields, 'a custom stop date field should be set up')
        date_fields = self.env['ir.model.fields'].search([('name', 'like', 'x_workflow_studio_date'), ('model_id', '=', model.id)])
    ...
    def test_10_model_option_value(self):
        ...
        self.assertEqual(len(extra_models), 0, 'no extra model should have been created')
        fields = self.env[model.model]._fields
        self.assertIn('x_workflow_studio_currency_id', fields, 'a custom currency field should be set up')
        self.assertIn('x_workflow_studio_currency_id', fields, 'a custom value field should be set up')
        value_field = self.env['ir.model.fields'].search([('name', '=', 'x_workflow_studio_value'), ('model_id', '=', model.id)])
        self.assertTrue(value_field.tracking, 'the x_workflow_studio_value field should be tracked')
        main_company = self.env.ref('base.main_company')
        default = self.env['ir.default']._get(model.model, 'x_workflow_studio_currency_id', company_id=main_company.id)
        self.assertEqual(default, main_company.currency_id.id, 'the default value for the x_workflow_studio_currency_id should be set')
        new_company = self.env['res.company'].create({'name': 'SpaceY', 'currency_id': self.env.ref('base.INR').id})
        new_default = self.env['ir.default']._get(model.model, 'x_workflow_studio_currency_id', company_id=new_company.id)
    ...
    def test_11_model_option_image(self):
        ...
        self.assertEqual(len(extra_models), 0, 'no extra model should have been created')
        fields = self.env[model.model]._fields
        self.assertIn('x_workflow_studio_image', fields, 'a custom image field should be set up')
    ...
    def test_12_model_option_stages(self):
        ...
        self.assertIn('x_workflow_studio_sequence', stage_fields, 'stages should always have a sequence even when the option is not ticked')
        fields = self.env[model.model]._fields
        self.assertNotIn('x_workflow_studio_sequence', fields, 'no custom sequence field is set up when the sequence option is not ticked')
        self.assertIn('x_workflow_studio_stage_id', fields, 'a custom stage field should be set up')
        self.assertIn('x_workflow_studio_priority', fields, 'a custom priority field should be set up')
        self.assertIn('x_color', fields, 'a custom color field should be set up')
        self.assertIn('x_workflow_studio_kanban_state', fields, 'a custom kanban state field should be set up')
        auto_stage = self.env[extra_model.model].search([])
        default = self.env['ir.default']._get(model.model, 'x_workflow_studio_stage_id')
        self.assertEqual(default, auto_stage.ids[0], 'the default stage should be set')
        stage_field = self.env['ir.model.fields'].search([('name', '=', 'x_workflow_studio_stage_id'), ('model_id', '=', model.id)])
        self.assertTrue(stage_field.tracking, 'the x_workflow_studio_stage_id field should be tracked')
        list_view = self.env["ir.ui.view"].search([("model", "=", model.model), ("type", "=", "list")], limit=1)
        kanban_view = self.env["ir.ui.view"].search([("model", "=", model.model), ("type", "=", "kanban")], limit=1)
        tree = etree.fromstring(list_view.get_combined_arch())
        self.assertEqual(tree.attrib['default_order'], 'x_workflow_studio_priority desc,id desc')
    ...
    def test_13_model_option_tags(self):
        ...
        fields = self.env[model.model]._fields
        self.assertIn('x_workflow_studio_tag_ids', fields, 'a custom tags field should be set up')
    ...
    def test_16_next_relation(self):
        ...
        IrModelFields = self.env["ir.model.fields"].with_context(workflow_studio=True)
    ...
    def test_19_custom_model_security(self):
        ...
        model_id, name = self.env['ir.model'].with_context(workflow_studio=True).name_create('X_Rockets')
        acl_admin = self.env['ir.model.access'].search([
            ('model_id', '=', model_id),
            ('group_id', '=', self.env.ref('base.group_system').id)
        ])
            ...
            IrModelFields.create(
                {
                    "ttype": "char",
                    "model_id": self.source_model.id,
                    "name": "x_workflow_studio_hello___hap",
                }
            )
    ...
    def test_22_rename_window_action(self):
        ...
        new_menu = self.env['ir.ui.menu'].with_context(workflow_studio=True).create({
            'name': 'Rockets',
            'action': action_ref,
        })
    ...
    def test_23_model_option_stages_with_sequence(self):
        ...
        self.assertEqual(len(extra_model), 1, 'an extra model should have been created for stages')
        stage_fields = self.env[extra_model.model]._fields
        self.assertIn('x_workflow_studio_sequence', stage_fields, 'stages should always have a sequence')
        fields = self.env[extra_model.model]._fields
        self.assertIn('x_workflow_studio_sequence', fields, 'a custom sequence field is set up for the main model')
        list_view = self.env["ir.ui.view"].search([("model", "=", model.model), ("type", "=", "list")], limit=1)
        kanban_view = self.env["ir.ui.view"].search([("model", "=", model.model), ("type", "=", "kanban")], limit=1)
        tree = etree.fromstring(list_view.get_combined_arch())
        self.assertEqual(tree.attrib['default_order'], 'x_workflow_studio_priority desc,x_workflow_studio_sequence asc,id desc')
        tree = etree.fromstring(kanban_view.get_combined_arch())
        self.assertEqual(tree.attrib['default_order'], 'x_workflow_studio_priority desc,x_workflow_studio_sequence asc,id desc')
        sequence_field = tree.xpath("//field[@name='x_workflow_studio_sequence']")[0]
        self.assertEqual(sequence_field.attrib, {'name': 'x_workflow_studio_sequence', 'widget': 'handle', 'invisible': '1'})

    def test_24_model_option_has_approval_bootstrap(self):
        model, _extra_models = self.env["ir.model"].studio_model_create(
            "Approval Rockets",
            options=["has_approval"],
        )
        self.assertTrue(model.is_approval, "Model should be configured as approval-enabled")

        category = self.env["workflow.approval.category"].search(
            [("res_model", "=", model.id)],
            limit=1,
        )
        self.assertTrue(category, "A default approval category should be created")
        self.assertTrue(category.version_ids, "A default workflow version should be created")
        self.assertTrue(category.active_version_id, "The default version should be active")
        ...
        self.assertTrue(
            inherited_form,
            "Approval-enabled models should get a form view inheriting the base approval request form",
        )

        approval_user_group = self.env.ref("workflow_engine.group_workflow_approval_user")
        acl = self.env["ir.model.access"].search(
            [("model_id", "=", model.id), ("group_id", "=", approval_user_group.id)],
            limit=1,
        )
        self.assertTrue(acl, "Approval users should get default model access")

        rule = self.env["ir.rule"].search(
            [("model_id", "=", model.id), ("name", "=", f"{model.name}: Request Owner Rule")],
            limit=1,
        )
        self.assertTrue(rule, "Approval-enabled models should get the request-owner rule")
    ...
    def test_25_model_option_transient_creates_wizard_action(self):
        model, _extra_models = self.env["ir.model"].studio_model_create(
            "Rockets Wizard",
            options=["is_transient"],
        )
        self.assertTrue(model.transient, "Model should be transient")
        action = model._create_default_action("Rockets Wizard")
        self.assertEqual(action.view_mode, "form")
        self.assertEqual(action.target, "new")

    def test_26_model_option_transient_approval_is_invalid(self):
    ...
    def test_performance_01_fields_batch(self):
        ...
        with patch('odoo.modules.registry.Registry._setup_models__', new=_setup_models):
            ...
            self.env['ir.model'].with_context(workflow_studio=True).studio_model_create('PerformanceIssues', options=OPTIONS_WL)
    ...
    def test_update_xmlid(self):
        ...
        with self.assertQueryCount(1):
            self.env['ir.model.data'].with_context(workflow_studio=True)._update_xmlids([
                {'xml_id': 'workflow_studio.xmlid', 'record': record}
            ])
    ...
    def test_ir_default_company_fields(self):
        model = self.env['ir.model'].create({
            ...
            'model': 'x_rockets',
            'field_id': [
                Command.create({'name': 'x_workflow_studio_company_id', 'ttype': 'many2one', 'relation': 'res.company'}),
...
class TestStudioIrModelHardcoded(TransactionCase):
    def test_23_export_hardcoded_models_and_fields(self):
        ...
        if not module_names.issuperset(needed_modules):
            missing = sorted(needed_modules - module_names)
            self.skipTest("Skipped hardcoded export checks; missing addon definitions: %s" % ", ".join(missing))
        ...
        for model, defaults in PRESET_MODELS_DEFAULTS:
            ...
            for field in defaults:
                self.assertIn(field, self.env["workflow.studio.export.model"]._fields)
