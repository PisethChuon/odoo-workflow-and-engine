...
from odoo.addons.workflow_studio.controllers.export import StudioExporter
from odoo.addons.workflow_studio.wizard.studio_export_wizard import _find_circular_dependencies, FIELDS_TO_EXPORT
...
class TestExport(HttpCase):
    ...
    def test_export_currency_field(self):
        ...
        data = self.env['ir.model.data'].search([
            ('workflow_studio', '=', True), ("model", "=", "ir.model.fields"), ("res_id", "in", (currency_field | monetary).ids)
        ])
        data = self.env["workflow.studio.export.wizard.data"].create(
            [{"model": d.model, "res_id": d.res_id, "workflow_studio": d.workflow_studio} for d in data]
        )
        wizard = self.env['workflow.studio.export.wizard'].create({
    ...
    def test_export_automation(self):
        ba = self.env["base.automation"].with_context(workflow_studio=True).create({
            "name": "Cron BaseAuto",
            "trigger": "on_time",
            "model_id": self.env.ref("base.model_res_users").id,
        })
        data = self.env['ir.model.data'].search([
            ('workflow_studio', '=', True), ("model", "=", "base.automation"), ("res_id", "=", ba.id)
        ])
    ...
    def test_export_unarchived_records(self):
        IrFilter = self.env['ir.filters'].with_context(workflow_studio=True)
    ...
    def test_fields_to_export_are_not_excluded(self):
        for model, fields_to_export in FIELDS_TO_EXPORT.items():
            export_model = self.env["workflow.studio.export.model"].create({
...
@unittest.skip("Legacy export UI tour relies on generic Studio navbar, unavailable in workflow-only mode.")
class TestExportTours(HttpCase):
    def test_can_export_new_module(self):
        exported_field = self.env["ir.model.fields"].with_context(workflow_studio=True).create(
            {
                "name": "x_export_tour_field",
                "model_id": self.env["ir.model"]._get("res.partner").id,
                "ttype": "char",
            }
        )
        self.start_tour(
            "/odoo?debug=tests",
            "can_export_new_module",
            login="admin",
        )
        # check the export result made by the tour
        wizard = self.env['workflow.studio.export.wizard'].search([], limit=1, order='id desc')
        ...
        self.assertIn("ir.model.fields", exported_models)
        exported_field_ids = data.filtered(lambda r: r.model == "ir.model.fields").mapped("res_id")
        self.assertIn(exported_field.id, exported_field_ids)
