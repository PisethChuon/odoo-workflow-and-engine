...
export const MODES = {
    EDITOR: "editor", HOME_MENU: "home_menu", APP_CREATOR: "app_creator",
};
...
const SUPPORTED_VIEW_TYPES = {
    [WORKFLOW_STUDIO_ALLOWED_VIEW_TYPE]: _t("Form"),
};
...
function normalizeEditedViewType(viewType, {fallback = null} = {}) {
    return isViewEditable(viewType) ? viewType : fallback;
}

function normalizeStudioMode(mode) {
    return mode === MODES.APP_CREATOR ? MODES.HOME_MENU : mode;
}

function isWorkflowStudioEditable(action) {
    // we allow only the workflow.approval.category model to activate the studio editable.
    if (!action.context?.active_model && action.context?.active_model !== 'workflow.approval.category') {
        return false;
    }
...
function isActionStateValid(actionState) {
    return (actionState.action || (actionState.model && (actionState.resId || actionState.view_type === "form")));
}

function defaultEditorTab(action) {
    const context = action?.context || {};
    if (context.workflow_category_id || context.workflow_version_id) {
        return "bpmn";
    }
    if (action?.res_model === "workflow.approval.category" || action?.res_model === "workflow.approval.category.version") {
        return "bpmn";
    }
    return "views";
}
...
function defaultActionForModel({model, resId, view_id, view_type}) {
    const action = {
        res_model: model, type: "ir.actions.act_window", res_id: resId,
    };
    if (view_type === "form" || resId) {
        action.views = [[view_id || false, WORKFLOW_STUDIO_ALLOWED_VIEW_TYPE]];
    } else {
        action.views = [[view_id || false, WORKFLOW_STUDIO_ALLOWED_VIEW_TYPE]];
    }
    return action;
...
function parseUrlIds(value) {
    if (Array.isArray(value)) {
        return value
            .flatMap((item) => parseUrlIds(item))
...
export const studioService = {
    dependencies: ["action", "home_menu", "menu", "notification", "orm"], async start(env, {menu, notification, orm}) {
        ...
        function syncStudioBodyClass() {
            if (typeof document === "undefined" || !document.body) {
                return;
            }
            document.body.classList.toggle("o_in_workflow_studio", Boolean(inStudio));
        }

        // Guard against a stale class when Studio is not active.
        syncStudioBodyClass();
            ...
            if (!inStudio) {
                ...
                try {
                    ...
                    if (e instanceof NotEditableActionError) {
                        notification.add(_t("This action is not editable by Workflow Studio"), {
                            type: "danger",
                        });
                        return;
                    }
        ...
        async function _loadParamsFromURL() {
            ...
            if (urlState.action !== "workflow_studio") {
                return;
            }

            state.studioMode = normalizeStudioMode(urlState[URL_MODE_KEY]);
            state.editedViewType = normalizeEditedViewType(urlState[URL_VIEW_KEY], {
                fallback: null,
            });
            ...
            if (state.studioMode === MODES.EDITOR || (!state.studioMode && routerActionState)) {
                const {
                    active_id, active_ids, active_model, workflow_category_id, workflow_version_id,
                } = urlState;
                if (active_id) {
                    additionalContext.active_id = parseUrlId(active_id);
                    additionalContext.active_ids = [parseUrlId(active_id)];
                }
                if (active_ids) {
                    const parsedActiveIds = parseUrlIds(active_ids);
                    if (parsedActiveIds.length) {
                        additionalContext.active_ids = parsedActiveIds;
                        if (!additionalContext.active_id) {
                            additionalContext.active_id = parsedActiveIds[0];
                        }
                    }
                }
                if (active_model) {
                    additionalContext.active_model = active_model;
                }
                if (workflow_category_id) {
                    additionalContext.workflow_category_id = parseUrlId(workflow_category_id);
                }
                if (workflow_version_id) {
                    additionalContext.workflow_version_id = parseUrlId(workflow_version_id);
                }
                if (routerActionState?.action || routerActionState?.model) {
                    const {action, model, resId} = routerActionState;
                    if (action) {
                        state.editedAction = await env.services.action.loadAction(action, additionalContext);
                    } else if (model) {
                        state.editedAction = defaultActionForModel(routerActionState);
                        state.editedAction.context = {
                            ...(state.editedAction.context || {}), ...additionalContext,
                        };
                    }
                    const resIds = getResIds(resId);
                    state.editedControllerState = {
                        resId: resIds[0], resIds,
                    };
                    state.studioMode = state.studioMode || MODES.EDITOR;
                    state.editorTab = state.editorTab || defaultEditorTab(state.editedAction);
                    if (!state.editedViewType) {
                        const candidateViewType = resId ? WORKFLOW_STUDIO_ALLOWED_VIEW_TYPE : state.editedAction.views?.[0]?.[1];
                        state.editedViewType = normalizeEditedViewType(candidateViewType, {
                            fallback: WORKFLOW_STUDIO_ALLOWED_VIEW_TYPE,
                        });
                    }
                }
            }
            if (!state.editedAction || !isWorkflowStudioEditable(state.editedAction)) {
                state.studioMode = [undefined, MODES.EDITOR].includes(state.studioMode) ? MODES.HOME_MENU : state.studioMode;
        ...
        async function _openStudio(targetMode, action = false, viewType = false) {
            ...
            targetMode = normalizeStudioMode(targetMode);
            ...
            if (targetMode === MODES.EDITOR) {
                ...
                if (!action) {
                    ...
                    if (currentController) {
                        ...
                        action = {
                            ...currentController.action, globalState: currentController.getGlobalState?.() || {},
                        };
                        viewType = currentController.view.type;
                        controllerState = Object.assign({}, currentController.getLocalState());
                        controllerState.resIds = getResIds(action.globalState.resIds || controllerState.resId);
                    }
                } else {
                    let toLoad;
                    ...
                    if (action.id) {
                        ...
                        toLoad = {
                            displayName: action.name, model: action.res_model,
                        };
                ...
                if (!isWorkflowStudioEditable(action)) {
                ...
                state.editedViewType = normalizeEditedViewType(vtype, {
                    fallback: WORKFLOW_STUDIO_ALLOWED_VIEW_TYPE,
                });
                state.editorTab = defaultEditorTab(action);
            ...
            try {
                actionStack = actionStack.filter((a) => !["menu", "workflow_studio"].includes(a.action));
                actionStack.push({
                    action: "workflow_studio",
        ...
        async function leave(actionId) {
            ...
            if (!actionId && state.studioMode === MODES.EDITOR && actionStack.length) {
                ...
                if (editedViewType) {
                    ...
                    } else if (lastAction.view_type === "form" && areActionStatesSimilar(lastAction, actionStack.at(-2))) {
                        actionStack.pop();
                    } else {
        ...
        async function reload(params = {}, reset = true) {
            ...
            if (actionContext.active_ids) {
                const parsedActiveIds = parseUrlIds(actionContext.active_ids);
                if (parsedActiveIds.length) {
                    additionalContext = Object.assign(additionalContext || {}, {
                        active_ids: parsedActiveIds,
                    });
                }
            }
            if (actionContext.active_model) {
                additionalContext = Object.assign(additionalContext || {}, {
                    active_model: actionContext.active_model,
                });
            }
            if (actionContext.workflow_category_id) {
                additionalContext = Object.assign(additionalContext || {}, {
                    workflow_category_id: actionContext.workflow_category_id,
                });
            }
            if (actionContext.workflow_version_id) {
                additionalContext = Object.assign(additionalContext || {}, {
                    workflow_version_id: actionContext.workflow_version_id,
                });
            }
            const action = await env.services.action.loadAction(state.editedAction.id, additionalContext);
        ...
        function toggleHomeMenu() {
            ...
            const targetMode = [MODES.APP_CREATOR, MODES.EDITOR].includes(state.studioMode) ? MODES.HOME_MENU : MODES.EDITOR;
        ...
        function pushState() {
            ...
            if (state._pushActionState) {
                ...
                const actionStack = [{action: this.editedAction.path || this.editedAction.id}, {action: "workflow_studio"},];
            ...
            const activeIds = parseUrlIds(state.editedAction?.context?.active_ids);
            if (activeIds.length) {
                search.active_ids = activeIds.join(",");
            }
            if (state.editedAction?.context?.active_model) {
                search.active_model = state.editedAction.context.active_model;
            }
            if (state.editedAction?.context?.workflow_category_id) {
                search.workflow_category_id = state.editedAction.context.workflow_category_id;
            }
            if (state.editedAction?.context?.workflow_version_id) {
                search.workflow_version_id = state.editedAction.context.workflow_version_id;
            }
        ...
        function setParams(params = {}, reset = true) {
            if ("mode" in params) {
                state.studioMode = normalizeStudioMode(params.mode);
            }
            if ("viewType" in params) {
                state.editedViewType = normalizeEditedViewType(params.viewType, {
                    fallback: WORKFLOW_STUDIO_ALLOWED_VIEW_TYPE,
                });
            }
            if ("action" in params) {
        ...
        env.bus.addEventListener("ACTION_MANAGER:UI-UPDATED", (ev) => {
            ...
            inStudio = Boolean(action && action.tag === "workflow_studio");
            syncStudioBodyClass();
        });

        const IrModelInfo = new Cache(async (model) => await orm.call("ir.model", "studio_model_infos", [model], {
            context: user.context,
        }), (model) => model);

        return {
            MODES, bus, isWorkflowStudioEditable() {
                const action = _getCurrentAction();
                return action ? isWorkflowStudioEditable(action) : false;
            }, open, reload, pushState, leave, toggleHomeMenu, setParams, get ready() {
                return studioProm;
            }, get mode() {
...
registry.category("services").add("workflow_studio", studioService);

export function useStudioServiceAsReactive() {
    const studio = useService("workflow_studio");
...
function actionLeave(env, action) {
    ...
    return env.services.workflow_studio.leave(actionId);
}
