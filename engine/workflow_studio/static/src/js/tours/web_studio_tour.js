...
import utils from "@workflow_studio/utils";
...
registry.category("web_tour.tours").add("web_studio_new_app_tour", {
    ...
    steps: () => [
        ...
        {
            trigger: ".o_home_menu_background, body.o_in_workflow_studio",
            timeout: 60000,
        },
        {
            trigger: "body",
            run: () => {
                if (!document.body.classList.contains("o_in_workflow_studio")) {
                    document.querySelector(".o_main_navbar .o_web_studio_navbar_item")?.click();
                }
            },
        },
        {
        ...
        },
        {
            ...
            content: markup(
                _t(
                    "Let's check the result. Close Workflow Studio to get an <b>overview of your app</b>."
                )
            ),
        ...
        },
        {
            ...
            content: markup(
                _t(
                    "Wow, nice! And I'm sure you can make it even better! Use this icon to open <b>Workflow Studio</b> and customize any screen."
                )
            ),
            tooltipPosition: "bottom",
            run: "click",
        ...
        {
            trigger:
                ".o_web_studio_breadcrumb .o_back_button, .o_web_studio_menu .o_menu_sections button:contains(Views)",
            run: () => {
                const breadcrumbBack = document.querySelector(
                    ".o_web_studio_breadcrumb .o_back_button"
                );
                if (breadcrumbBack) {
                    breadcrumbBack.click();
                    return;
                }
                const legacyViewsButton = [...document.querySelectorAll(
                    ".o_web_studio_menu .o_menu_sections button"
                )].find((button) => button.textContent.trim() === "Views");
                legacyViewsButton?.click();
            },
        },
        {
            trigger: ".o_web_studio_views, .o_web_studio_view_category",
            timeout: 20000,
        },
        {
            trigger: "body",
            run: () => {
                // Some studio builds only expose the Form card in the views dashboard.
                // Click Kanban when available, otherwise keep the flow on the dashboard.
                document
                    .querySelector(
                        ".o_web_studio_view_category .o_web_studio_thumbnail_kanban, .o_web_studio_thumbnail_item.o_web_studio_thumbnail_kanban"
                    )
                    ?.click();
            },
        },
