...
import { useApproval } from "@workflow_studio/approval/approval_hook";
import { StudioApproval } from "@workflow_studio/approval/studio_approval";

registry.category("services").add("clear_caches_on_approval_rules_change", {
    start() {
        rpcBus.addEventListener("RPC:RESPONSE", (ev) => {
            const { model, method } = ev.detail.data.params;
            if (["workflow.studio.approval.rule"].includes(model) && UPDATE_METHODS.includes(method)) {
