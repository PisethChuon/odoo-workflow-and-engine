...
export const getApprovalSpecBatchedService = {
    name: "workflow_studio.get_approval_spec_batched",
    dependencies: ["orm"],
    start(env, { orm }) {
        return batched(async (...argsLists) => {
            const approvals = await orm.silent.call("workflow.studio.approval.rule", "get_approval_spec", [
                argsLists.flat(),
            ]);
...
class StudioApproval {
    ...
    async checkApproval() {
        ...
        const state = this._getState();
        state.syncing = true;
        const result = await this.orm.call("workflow.studio.approval.rule", "check_approval", args);
    ...
    async setApproval(ruleId, approved) {
        try {
            await this.orm.call("workflow.studio.approval.rule", "set_approval", [[ruleId]], {
                res_id: this.resId,
                approved,
            });
        } catch (e) {
    ...
    async cancelApproval(ruleId) {
        try {
            await this.orm.call("workflow.studio.approval.rule", "delete_approval", [[ruleId]], {
                res_id: this.resId,
            });
        } catch (e) {
            this.fetchApprovals();
...
export function useApproval({ getRecord, method, action }) {
    ...
    const getApprovalSpecBatched = useEnv().services["workflow_studio.get_approval_spec_batched"];
