...
import { useEditorBreadcrumbs } from "@workflow_studio/client_action/editor/edition_flow";
import { KeepLast } from "@web/core/utils/concurrency";
import { renderToMarkup } from "@web/core/utils/render";
import { makeActiveField } from "@web/model/relational_model/utils";
import { humanReadableError } from "@workflow_studio/client_action/report_editor/utils";

const notificationErrorTemplate = "workflow_studio.ReportEditor.NotificationError";
...
export class ReportEditorModel extends Reactive {
    ...
    async loadReportData() {
        ...
        const data = await rpc("/workflow_studio/load_report_editor", {
    ...
    async loadReportQweb() {
        ...
        try {
            const reportQweb = await this.loadHtmlKeepLast.add(
                rpc("/workflow_studio/get_report_qweb", {
                    report_id: this.editedReportId,
                    context: this.routesContext,
                })
            );
    ...
    async loadReportHtml({ resId } = {}) {
        ...
        try {
            const reportHtml = await this.loadHtmlKeepLast.add(
                rpc("/workflow_studio/get_report_html", {
                    report_id: this.editedReportId,
    ...
    async resetReport(includeHeaderFooter = true) {
        this.setInEdition(true);
        await rpc("/workflow_studio/reset_report_archs", {
            report_id: this.editedReportId,
...
export function useReportEditorModel() {
    ...
    const env = useEnv();
    services.studio = { ...env.services.workflow_studio };
