...
export class HomeMenuCustomizer extends Component {
    static template = "workflow_studio.HomeMenuCustomizer";
    static props = {};
    ...
    setBackgroundImage(attachment_id) {
        return rpc("/workflow_studio/set_background_image", {
     ...
     */
    exportCusto() {
        this.actionManager.doAction("workflow_studio.action_studio_export_wizard");
    }
    /**
    ...
    async confirmReset() {
        this.ui.block();
        try {
            await rpc("/workflow_studio/reset_background_image", {
                context: user.context,
            });
