...
import { viewTypeToString } from "@workflow_studio/studio_service";
import {
    xpathToLegacyXpathInfo,
    cleanClickedElements,
} from "@workflow_studio/client_action/view_editor/editors/utils";
import { Reactive, getFieldsInArch, memoizeOnce } from "@workflow_studio/client_action/utils";
import { getModifier, resetViewCompilerCache } from "@web/views/view_compiler";
import { _t } from "@web/core/l10n/translation";
import { EditorOperations, SnackbarIndicator } from "@workflow_studio/client_action/editor/edition_flow";
...
export class ViewEditorModel extends Reactive {
    constructor({ env, services, editionFlow, viewRef, initialState = {} }) {
        ...
        this._getControllerProps = memoizeOnce(function () {
            ...
            const controllerClass = this.editorInfo?.editor?.Controller;
            const supportsParentRecord = Boolean(
                controllerClass &&
                    controllerClass.props &&
                    Object.prototype.hasOwnProperty.call(controllerClass.props, "parentRecord")
            );
            if (
                ["list", "form"].includes(this.viewType) &&
                this.mode === "interactive" &&
                this._subviewInfo &&
                supportsParentRecord
            ) {
        ...
        this.__getDefaultStudioViewProps = memoizeOnce(() => {
            ...
            return {
                context: { ...context, workflow_studio: 1 },
    ...
    async renameField(fieldName, newName, { label, autoUnique = true } = {}) {
        ...
        if (!newName.startsWith("x_workflow_studio_")) {
            newName = `x_workflow_studio_${newName}`;
        }
        ...
        const prom = rpc("/workflow_studio/rename_field", {
    ...
    async _createInlineView({ subViewType, fullXpath, subViewRef, resModel, fieldName }) {
        ...
        // of ir.ui.view
        const context = { ...user.context, lang: false, workflow_studio: true };
        if (subViewRef) {
            context[`${subViewType}_view_ref`] = subViewRef;
        }

        // FIXME: maybe this route should return def _return_view
        const studioViewArch = await this._decoratedRpc("/workflow_studio/create_inline_view", {
            model: resModel,
    ...
    async _editView(operations) {
        ...
        return rpc("/workflow_studio/edit_view", {
            view_id: this.mainView.id,
            studio_view_arch: this.studioViewArch,
    ...
    async _editViewArch(viewId, viewArch) {
        ...
        const result = await rpc("/workflow_studio/edit_view_arch", {
            view_id: viewId,
            view_arch: viewArch,
            // We write views in the base language to make sure we do it on the source term field
            // of ir.ui.view
    ...
    async _getStudioViewArch() {
        const result = await rpc("/workflow_studio/get_studio_view_arch", {
            model: this.resModel,
...
export function useEditNodeAttributes({ isRoot = false } = {}) {
  ...
  function normalizeAttrs(attrs) {
    ...
    // Ensure options is always a string (Odoo views store it as string)
    if (out.options && typeof out.options === "object") {
      out.options = JSON.stringify(out.options);
    }

    // Also protect against accidental "[object Object]"
    if (out.options === "[object Object]") {
      out.options = "{}";
    }
  ...
  function editNodeAttributes(newAttributes) {
    ...
    if (isRoot) {
      ...
      target = vem.getFullTarget(vem.activeNodeXpath);
      const { arch, attrs } = vem.activeNode;
      node = { tag: arch.tagName, attrs };
    }

    const operation = {
      new_attrs: normalizeAttrs(newAttributes),
      type: "attributes",
      position: "attributes",
      target,
    };
