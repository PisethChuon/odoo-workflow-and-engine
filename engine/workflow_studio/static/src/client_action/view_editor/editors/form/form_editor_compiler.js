...
export class FormEditorCompiler extends formView.Compiler {
    compile(key, params = {}) {
        ...
        const compiledHasChatter = Boolean(
            compiled.querySelector(
                ".o-mail-Form-chatter, .o-mail-Chatter, .o_Studio_ChatterContainer, [data-studio-xpath][data-studio-xpath*='/chatter']"
            )
        );
        if (compiledHasChatter) {
    ...
    compileNode(node, params = {}, evalInvisible = true) {
        ...
        if (nodeType === 1) {
            // Avoid rendering a duplicated chatter in Studio:
            // some legacy form views still define chatter via message/activity fields
            // or an `oe_chatter` wrapper instead of the modern `<chatter/>` node.
            if (
                node.classList?.contains("oe_chatter") ||
                node.classList?.contains("o-mail-Form-chatter")
            ) {
                this.addChatter = false;
            }
