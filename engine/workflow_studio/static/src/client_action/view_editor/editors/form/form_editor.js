...
const FIELD_FRIENDLY_HOOK_TYPES = new Set(["genericTag", "afterGroup", "afterNotebook", "insideSheet"]);

const getDraggingStructure = (draggedEl) =>
    draggedEl.dataset.structure || draggedEl.dataset.studioStructure || "field";

const isBlackListedHook = (draggedEl, hookEl) => {
    const draggingStructure = getDraggingStructure(draggedEl);
    // Regular fields are valid on generic/container hooks; this is required for
    // heavily customized forms (div/row/col layouts) where those are the only drop targets.
    if (draggingStructure === "field" && FIELD_FRIENDLY_HOOK_TYPES.has(hookEl.dataset.type)) {
        return false;
    }
    return (
...
function isValidFormHook({ hook, element }) {
    const draggingStructure = getDraggingStructure(element);
