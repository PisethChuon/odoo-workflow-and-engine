...
import {
    ...
    randomName,
} from "@workflow_studio/client_action/view_editor/editors/utils";
...
import {
    ...
    RelatedChainBuilder,
} from "@workflow_studio/client_action/view_editor/interactive_editor/field_configuration/field_configuration";
import {
    computeXpath,
    getNodesFromXpath,
    countPreviousSiblings,
} from "@workflow_studio/client_action/view_editor/editors/xml_utils";
import { DefaultViewSidebar } from "@workflow_studio/client_action/view_editor/default_view_sidebar/default_view_sidebar";
...
export class InteractiveEditor extends Component {
    static template = "workflow_studio.InteractiveEditor";
    static components = {};
    static props = {
        editor: true,
        slots: { type: Object },
    ...
    async addStructure(structure, droppedData, targetInfo) {
        ...
        const normalizedTargetInfo =
            structure === "field" ? this.normalizeFieldTargetInfo(targetInfo) : targetInfo;
        const operation = {
            target:
                _operation?.target ||
                (normalizedTargetInfo.xpath
                    ? this.viewEditorModel.getFullTarget(normalizedTargetInfo.xpath)
                    : undefined),
            position: normalizedTargetInfo.position,
            type: "add",
            ..._operation,
        };
        this.setAutoClick(normalizedTargetInfo, operation.node);
        return this.viewEditorModel.doOperation(operation);
    }

    normalizeFieldTargetInfo(targetInfo) {
        if (!targetInfo?.xpath) {
            return targetInfo;
        }
        const [targetNode] = getNodesFromXpath(targetInfo.xpath, this.viewEditorModel.xmlDoc);
        if (!targetNode) {
            return targetInfo;
        }
        const forceInsideContainer = new Set([
            "genericTag",
            "afterGroup",
            "afterNotebook",
            "insideSheet",
        ]);
        if (["group", "page", "sheet", "notebook"].includes(targetNode.tagName)) {
            if (
                targetInfo.position !== "inside" ||
                forceInsideContainer.has(targetInfo.type)
            ) {
                return {
                    ...targetInfo,
                    xpath: computeXpath(targetNode, this.viewEditorModel.viewType),
                    position: "inside",
                };
            }
            return targetInfo;
        }
        if (targetNode.tagName === "field") {
            return targetInfo;
        }
        let semanticContainer = targetNode.parentElement;
        while (
            semanticContainer &&
            !["group", "page", "sheet", "notebook"].includes(semanticContainer.tagName)
        ) {
            semanticContainer = semanticContainer.parentElement;
        }
        ...
        return {
            ...targetInfo,
            xpath: computeXpath(semanticContainer, this.viewEditorModel.viewType),
            position: "inside",
        };
    }
    ...
    async getNewFieldNode(data) {
        ...
        const newNode = {
            field_description: {
                ...
                name: randomName(`x_workflow_studio_${data.fieldType}_field`),
                type: data.fieldType,
                model_name: this.viewEditorModel.resModel,
                special: data.special,
            },
            tag: "field",
            attrs: { widget: data.widget, string },
        };
