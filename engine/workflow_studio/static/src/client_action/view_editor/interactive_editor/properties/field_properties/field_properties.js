...
class TechnicalName extends Component {
    ...
    static template = "workflow_studio.ViewEditor.InteractiveEditorProperties.Field.TechnicalName";
    static components = { Property };

    setup() {
        this.renameField = (value) =>
            this.env.viewEditorModel.renameField(this.props.node.attrs.name, `x_workflow_studio_${value}`, {
                autoUnique: false,
            });
    }
    ...
    get fieldName() {
        ...
        if (this.canEdit) {
            return fName.split("x_workflow_studio_")[1];
        }
        return fName;
    }
}
...
export class FieldProperties extends Component {
    static template = "workflow_studio.ViewEditor.InteractiveEditorProperties.Field";
    static props = {
        node: { type: Object },
    ...
    async onChangeDefaultValue(value) {
        await rpc("/workflow_studio/set_default_value", {
    ...
    async getDefaultValue(node) {
        const defaultValueObj = await rpc("/workflow_studio/get_default_value", {
            model_name: this.env.viewEditorModel.resModel,
    ...
    editSelectionChoices() {
        ...
        this.dialog.add(SelectionContentDialog, {
            ...
            onConfirm: async (choices) => {
                const result = await rpc("/workflow_studio/edit_field", {
                    model_name: this.env.viewEditorModel.resModel,
                ...
                if (result && result.records_linked) {
                    ...
                    await new Promise((resolve) => {
                        this.dialog.add(ConfirmationDialog, {
                            ...
                            confirm: async () => {
                                await rpc("/workflow_studio/edit_field", {
                                    model_name: this.env.viewEditorModel.resModel,
                                    field_name: field.name,
                                    values: { selection: JSON.stringify(choices) },
                                    force_edit: true,
