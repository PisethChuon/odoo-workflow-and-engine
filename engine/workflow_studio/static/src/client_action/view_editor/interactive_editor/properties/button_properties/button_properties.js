...
import { useSnackbarWrapper } from "@workflow_studio/client_action/view_editor/view_editor_hook";
...
import { buildApprovalKey } from "@workflow_studio/approval/approval_hook";
...
export class ButtonProperties extends Component {
    static template = "workflow_studio.ViewEditor.InteractiveEditorProperties.Button";
    static props = {
        node: { type: Object },
    ...
    setup() {
        ...
        this.recordProps = {
            resModel: "workflow.studio.approval.rule",
    ...
    async loadActions() {
        ...
        this.state.actionsList = await rpc("/workflow_studio/get_actions_for_model", {
    ...
    async onChangeApprovalRecord(record, changes, id) {
        await this.decoratedOrmWrite("workflow.studio.approval.rule", [id], changes);
    ...
    async createApprovalRule() {
        ...
        await this.decoratedOrmCall("workflow.studio.approval.rule", "create_rule", params);
        this.updateApprovalSpec();
    }
    ...
    async getApprovalSpec(approvalParams) {
        ...
        const approvals = await this.env.services["workflow_studio.get_approval_spec_batched"](
    ...
    async onApprovalArchive(id) {
        await this.decoratedOrmWrite("workflow.studio.approval.rule", [id], {
            active: false,
        });
        this.updateApprovalSpec();
    }
    ...
    async onApprovalEdit(name, id, value) {
        ...
        await rpc("/workflow_studio/edit_approval", {
            model: this.env.viewEditorModel.resModel,
