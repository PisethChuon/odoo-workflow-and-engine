...
export class StudioActionContainer extends Component {
    ...
    setup() {
        ...
        const doAction = async (action, options) => {
            try {
                ...
                if (action !== "workflow_studio.action_editor") {
    ...
    async getStudioAction() {
        ...
        if (editorTab === "views") {
            if (editedViewType) {
                return "workflow_studio.view_editor";
            }
            return tab.action;
        }
        if (tab.action) {
            ...
            return "workflow_studio.report_editor";
        } else {
            const action = await rpc("/workflow_studio/get_studio_action", {
