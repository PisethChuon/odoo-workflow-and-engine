...
import { Reactive } from "@workflow_studio/client_action/utils";
...
export class EditionFlow extends Reactive {
    ...
    async loadViews({ forceSearch = false } = {}) {
        ...
        const options = { loadIrFilters: true, loadActionMenus: false, id, workflow_studio: true };
    ...
    restoreDefaultView(viewId, viewType) {
        return new Promise((resolve) => {
            const confirm = async () => {
                ...
                const res = await rpc("/workflow_studio/restore_default_view", {
                    view_id: viewId,
                });
...
export class EditorOperations extends Reactive {
    ...
    get canUndo() {
        const operations = Array.isArray(this.operations) ? this.operations : [];
        const pending = Array.isArray(this.pending) ? this.pending : [];
        return operations.length || pending.length;
    }

    get canRedo() {
        const undone = Array.isArray(this.undone) ? this.undone : [];
        const pendingUndone = Array.isArray(this.pendingUndone) ? this.pendingUndone : [];
        return undone.length || pendingUndone.length;
    }
    ...
    _prepare(mode) {
        ...
        if (!pending) {
            this.pending = Array.isArray(raw.operations) ? [...raw.operations] : [];
            this.pendingUndone = Array.isArray(raw.undone) ? [...raw.undone] : [];
        }
        return true;
    }
    ...
    _close(done = null) {
        ...
        const pendingUndone = Array.isArray(raw.pendingUndone) ? raw.pendingUndone : [];
        this.pending = null;
