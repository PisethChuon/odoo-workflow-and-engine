...
import { ThumbnailItem } from "@workflow_studio/client_action/components/thumbnail_item/thumbnail_item";
import { viewTypeToString, useStudioServiceAsReactive } from "@workflow_studio/studio_service";
...
class ActionEditor extends Component {
    static template = "workflow_studio.ActionEditor";
    ...
    getOrderedViewTypes(viewTypes) {
        ...
        const viewInfos = viewTypes.map((viewType) => ({
            ...
            imgUrl: `/workflow_studio/static/src/img/view_type/${viewType}.png`,
    ...
    async addViewType(viewType) {
        ...
        let viewAdded = await rpc("/workflow_studio/add_view_type", {
        ...
        if (viewAdded !== true) {
            this.addDialog(AlertDialog, {
                body: _t("Creating this type of view is not currently supported in Studio."),
            });
            viewAdded = false;
        }
        if (viewAdded) {
    ...
    async editAction(changes) {
        await rpc("/workflow_studio/edit_action", {
...
registry.category("actions").add("workflow_studio.action_editor", ActionEditor);
