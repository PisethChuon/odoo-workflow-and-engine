...
import { edit, queryOne } from "@odoo/hoot-dom";
...
test("edit menu dialog rendering", async () => {
    ...
    expect(getComputedStyle(queryOne(".oe_menu_editor")).borderRadius).toBe("9px");
    expect(getComputedStyle(queryOne(".oe_menu_editor")).backgroundColor).toBe(
        "rgb(248, 249, 251)"
    );
});
...
test("edit menu dialog: create menu", async () => {
    ...
    onRpc("/workflow_studio/create_new_menu", async (request) => {
    ...
    expect(
        getComputedStyle(queryOne(".o_web_studio_model_configurator_option")).borderRadius
    ).toBe("9px");
    expect(
        getComputedStyle(queryOne(".o_web_studio_model_configurator_option")).backgroundColor
    ...
    expect.verifySteps(["/workflow_studio/create_new_menu", "/web/webclient/load_menus"]);
});
