...
import { EditionFlow } from "@workflow_studio/client_action/editor/edition_flow";
...
export function handleDefaultStudioRoutes() {
    ...
    onRpc("/workflow_studio/edit_view", () => {});
    onRpc("/workflow_studio/edit_view_arch", () => {});
    onRpc("/workflow_studio/get_actions_for_model", () => []);
    onRpc("/workflow_studio/get_default_value", () => ({ default_value: undefined }));
    onRpc("/workflow_studio/get_studio_view_arch", () => ({ studio_view_arch: "" }));
...
export async function mountViewEditor(params) {
    ...
    env.services.workflow_studio.setParams({
...
function prepareRegistry(filterRegistry) {
    ...
    const REQUIRED_SERVICES = [
        ...
        "ui",
        "effect",
        "workflow_studio.get_approval_spec_batched",
    ];
