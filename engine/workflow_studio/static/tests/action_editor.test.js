...
import { edit, press, queryOne } from "@odoo/hoot-dom";
...
test("action editor uses the shared Studio card and property rail treatment", async () => {
    ...
    expect(".o_web_studio_thumbnail_item").not.toHaveCount(0);
    expect(getComputedStyle(queryOne(".o_web_studio_thumbnail_item")).borderRadius).toBe(
        "9px"
    );
    expect(getComputedStyle(queryOne(".o_web_studio_thumbnail_item")).backgroundColor).toBe(
        "rgb(255, 255, 255)"
    );
...
test("add a gantt view", async () => {
    ...
    onRpc("/workflow_studio/add_view_type", async (request) => {
...
test("disable the view from studio", async () => {
    ...
    onRpc("/workflow_studio/edit_action", async () => {
        listIsDisabled = true;
        return true;
    });
...
test("add groups on action", async () => {
    onRpc("/workflow_studio/edit_action", async (request) => {
        const { params } = await request.json();
