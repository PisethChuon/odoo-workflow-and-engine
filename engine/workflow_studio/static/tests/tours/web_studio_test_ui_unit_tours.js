...
import { stepNotInStudio, assertEqual } from "@workflow_studio/../tests/tours/tour_helpers";
...
registry.category("web_tour.tours").add("web_studio_set_tree_node_conditional_invisibility", {
    steps: () => [
        ...
        {
            trigger: ".o_web_studio_sidebar, .o_web_studio_list_view_editor, .o_web_studio_views_icons a[title='List']",
            run: () => {
                const listShortcut = document.querySelector(".o_web_studio_views_icons a[title='List']");
                if (listShortcut && !document.querySelector(".o_web_studio_list_view_editor")) {
                    listShortcut.click();
                }
            },
        },
...
registry.category("web_tour.tours").add("web_studio_set_view_default_group_by", {
    steps: () => [
        ...
        {
            trigger: "body:not(.o_in_workflow_studio):has(.o_searchview_facet):has(.o_list_table)",
            run() {
...
registry.category("web_tour.tours").add("web_studio_test_related_file", {
    steps: () => [
        {
            trigger: "a[data-menu-xmlid='workflow_studio.studio_test_user_menu']",
            run: "click",
        },
        {
            trigger: ".o_form_view",
...
registry.category("web_tour.tours").add("web_studio_test_new_field_rename_description", {
    steps: () => [
        ...
        {
            trigger:
                ".o_web_studio_form_view_editor label[for='x_workflow_studio_my_new_field_0']:contains(my new field)",
        },
    ],
});
...
registry.category("web_tour.tours").add("workflow_studio.test_button_rainbow_effect", {
    steps: () => [
        {
            trigger: "a[data-menu-xmlid='workflow_studio.studio_test_partner_menu']",
            run: "click",
...
registry.category("web_tour.tours").add("web_studio_test_edit_duplicate_attribute_list", {
    steps: () => [
        ...
        {
            trigger: ".o_web_studio_sidebar, .o_web_studio_views_icons a[title='List']",
            run: () => {
                const listShortcut = document.querySelector(".o_web_studio_views_icons a[title='List']");
                if (listShortcut && !document.querySelector(".o_web_studio_list_view_editor")) {
                    listShortcut.click();
        ...
        {
            trigger: ".o_web_studio_sidebar .o_web_studio_view, .o_web_studio_property .form-check-label:contains(Can Create)",
            run: () => {
                document.querySelector(".o_web_studio_sidebar .o_web_studio_view")?.click();
            },
        },
        {
            trigger: ".o_web_studio_property .form-check-label:contains(Can Create)",
        },
        {
            content: "Uncheck 'create' property to hide 'duplicate'",
            trigger: ".o_web_studio_property .form-check-label:contains(Can Create)",
        ...
        {
            content: "'duplicate' property should be hidden, 4th item is show_visible",
            trigger: ".o_web_studio_property:nth-child(4) .form-check-label:contains(Show Invisible Elements)",
        },
        {
            content: "Check 'create' property to show 'duplicate'",
            trigger: ".o_web_studio_property .form-check-label:contains(Can Create)",
        ...
        {
            content: "'duplicate' property should be visible and the 4th item",
            trigger: ".o_web_studio_property:nth-child(4) .form-check-label:contains(Can Duplicate)",
            run: "click",
        },
        {
            trigger: ".o_web_studio_snackbar:not(:has(.fa-spin))",
    ...
    steps: () => [
        ...
        },
        {
            ...
            async run(helpers) {
                ...
                const cb = ({ detail }) => {
                    if (detail.url === "/workflow_studio/set_default_value") {
                        this.anchor.value = "";
...
registry.category("web_tour.tours").add("web_studio_test_automagically_added_fields", {
    steps: () => [
        ...
        {
            trigger: "body.o_in_workflow_studio .o_list_view",
        },
        {
            trigger: ".o_web_studio_sidebar .o_web_studio_view",
            run: "click",
        ...
        {
            trigger: "body.o_in_workflow_studio .o_list_view .o_web_studio_show_invisible",
            run: () => {
                const allTh = document.querySelectorAll(
                    "body.o_in_workflow_studio .o_list_view th:not(.o_web_studio_hook)"
                );
        ...
        {
            trigger: "body.o_in_workflow_studio .o_kanban_view",
        },
        {
            trigger: ".o_web_studio_sidebar .o_web_studio_view",
            run: "click",
        ...
        {
            trigger:
                "body.o_in_workflow_studio .o_kanban_record:has(.o_web_studio_show_invisible)",
            run() {
                const allInvisible = [
                    ...document.querySelectorAll(
                        "body.o_in_workflow_studio .o_kanban_record .o_web_studio_show_invisible"
                    ),
                ];
        ...
        {
            trigger: "body.o_in_workflow_studio .o_form_view",
        },
        {
            trigger: ".o_web_studio_sidebar .o_web_studio_view",
            run: "click",
        ...
        {
            trigger: "body.o_in_workflow_studio .o_form_view:has(.o_web_studio_show_invisible)",
            run() {
                const allInvisible = [
                    ...document.querySelectorAll(
                        "body.o_in_workflow_studio .o_form_view .o_web_studio_show_invisible"
                    ),
                ];
                if (allInvisible.length < 2) {
                    throw new Error(
                        `Expected at least 2 invisible markers in form view, got ${allInvisible.length}`
                    );
                }
                const values = [
                    ...new Set(
                ...
                const hasDisplayName = values.some(
                    (value) => value.includes("display_name") || value.includes("display name")
                );
                const hasName = values.some(
                    (value) =>
                        value === "name" || value.includes("name") || value.includes("name ?")
                );
                if (!(hasDisplayName && hasName)) {
                    throw new Error(
                        `Expected invisible fields display_name and name in form view, got ${values.toString()}`
                    );
                }
            },
        },
