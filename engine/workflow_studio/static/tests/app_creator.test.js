...
import { AppCreator } from "@workflow_studio/client_action/app_creator/app_creator";
...
test("app creator: standard flow with model creation", async () => {
    ...
    onRpc("/workflow_studio/create_new_app", async (request) => {
    ...
    expect("input[name='has_approval']").toHaveCount(1);
    expect("input[name='is_transient']").toHaveCount(1);
...
test("app creator: debug flow with existing model", async () => {
    ...
    onRpc("ir.model", "name_search", ({ kwargs }) => {
        expect(kwargs.domain).toContainEqual(["abstract", "=", false]);
        expect(kwargs.domain).toContainEqual(["!", ["id", "in", []]]);
