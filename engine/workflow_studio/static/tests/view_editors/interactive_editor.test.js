...
import { edit, press, queryOne, waitFor } from "@odoo/hoot-dom";
...
test("edit the currency of a monetary field", async () => {
    ...
    onRpc("/workflow_studio/set_currency", async (request) => {
        const { params } = await request.json();
...
test("add one2many field", async () => {
    ...
    onRpc("/workflow_studio/edit_view", () => expect.step("edit_view"));
...
test("add a many2many field", async () => {
    ...
    onRpc("/workflow_studio/edit_view", async (request) => {
        ...
        expect(fieldDescr.name).toMatch(/^x_workflow_studio_many2many.*/);
...
test("add a many2one field", async () => {
    ...
    onRpc("/workflow_studio/edit_view", async (request) => {
        ...
        expect.step("edit_view");
        const fieldDescr = params.operations[0].node.field_description;
        expect(fieldDescr.name).toMatch(/^x_workflow_studio_many2one.*/);
        delete fieldDescr.name;
        expect(fieldDescr).toEqual({
...
test("open XML editor in read-only", async () => {
    ...
    onRpc("/workflow_studio/get_xml_editor_resources", async (request) => {
        const { params } = await request.json();
    ...
    const xmlEditorStyle = getComputedStyle(queryOne(".o_web_studio_xml_editor"));
    expect(xmlEditorStyle.backgroundColor).toBe("rgb(32, 36, 43)");
    expect(xmlEditorStyle.minWidth).toBe("360px");
    expect(xmlEditorStyle.maxWidth).toBe("720px");
});
...
test("XML editor: reset operations stack", async () => {
    ...
    onRpc("/workflow_studio/edit_view_arch", async () => {
...
test("blockUI not removed just after rename", async () => {
    ...
    onRpc("/workflow_studio/rename_field", () => {
