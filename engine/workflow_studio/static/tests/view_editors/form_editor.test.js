...
import { COMPUTED_DISPLAY_OPTIONS } from "@workflow_studio/client_action/view_editor/interactive_editor/properties/type_widget_properties/type_specific_and_computed_properties";
...
import {
    ...
    mountViewEditor,
    openStudio,
} from "@workflow_studio/../tests/view_editor_tests_utils";
import { formEditor } from "@workflow_studio/client_action/view_editor/editors/form/form_editor";
...
test("Form editor should contains the view and the editor sidebar", async () => {
    ...
    expect(getComputedStyle(document.body).getPropertyValue("--wfs-sidebar-width").trim()).toBe(
        "332px"
    );
    expect(getComputedStyle(queryOne(".o_web_studio_sidebar")).width).toBe("332px");
    expect(getComputedStyle(queryOne(".o_web_studio_sidebar")).backgroundColor).toBe(
        "rgb(255, 255, 255)"
    );
    expect(".o_web_studio_sidebar .o_web_studio_component").not.toHaveCount(0);
    expect(
        getComputedStyle(queryOne(".o_web_studio_sidebar .o_web_studio_component")).borderRadius
    ).toBe("9px");
});
...
test("Form editor view buttons can be set to invisible", async () => {
    onRpc("/workflow_studio/edit_view", async (request) => {
...
test("image field edition (change size)", async () => {
    onRpc("/workflow_studio/edit_view", (request) => {
        const newArch = `
                <form>
                    <sheet>
...
test("'class' attribute is editable in the sidebar with a tooltip", async () => {
    const arch = `<form>
        ...
        <sheet>
            <field name="display_name" class="workflow-studio"/>
        </sheet>
    </form>
    `;
    onRpc("/workflow_studio/edit_view", async (request) => {
    ...
    expect(".o_web_studio_property input[id=class]").toHaveValue("workflow-studio");
    const tooltip =
...
test("TextField can edit its placeholder", async () => {
    ...
    await mountViewEditor({
        ...
        <sheet>
            <group>
                <field name="text" class="workflow-studio"/>
            </group>
        </sheet>
    </form>
    `,
...
test("One2Many can edit its placeholder", async () => {
    await mountViewEditor({
        ...
        <sheet>
            <group>
                <field name="product_ids" class="workflow-studio"/>
            </group>
        </sheet>
    </form>
    `,
...
test("edit one2many form view (2 level) and check chatter allowed", async () => {
    ...
    expect.verifySteps([
        "studio_model_infos",
        "get_views",
        "/workflow_studio/get_studio_view_arch",
        "web_read",
    ]);
    ...
    expect.verifySteps(["/workflow_studio/edit_view"]);
});
...
test("approval one rule by default", async () => {
    ...
    onRpc("/workflow_studio/edit_view", (request) => editView(request, "form", arch));

    onRpc("workflow.studio.approval.rule", "create_rule", ({ args }) => {
        expect(args).toEqual(["coucou", null, "0", "2"]);
        return {};
    });

    onRpc("workflow.studio.approval.rule", "write", () => ({}));

    onRpc("workflow.studio.approval.rule", "get_approval_spec", () => {
...
test("display one2many without inline views", async () => {
    ...
    onRpc("/workflow_studio/create_inline_view", async (request) => {
