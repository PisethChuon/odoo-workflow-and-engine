...
import { PivotEditorSidebar } from "@workflow_studio/client_action/view_editor/editors/pivot/pivot_editor";
...
test("default value in sidebar", async () => {
    ...
    onRpc("/workflow_studio/get_default_value", async (request) => {
...
test("remove starting underscore from new field value", async () => {
    ...
    onRpc("/workflow_studio/rename_field", () => true);
...
test("notebook and group not drag and drop in a group", async () => {
    ...
    onRpc("/workflow_studio/edit_view", () => expect.step("editView"));
...
test("open xml editor of component view", async () => {
    ...
    onRpc("/workflow_studio/get_xml_editor_resources", () => ({
        views: [
            {
                active: true,
                arch: "<form/>",
