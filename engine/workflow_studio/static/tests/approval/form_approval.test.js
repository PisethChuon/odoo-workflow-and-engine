...
test("approval can revoke below rules", async () => {
    onRpc("workflow.studio.approval.rule", "delete_approval", (args) => {
...
test("reload record when setting/deleting approval", async () => {
    onRpc("workflow.studio.approval.rule", "set_approval", ({ args, kwargs }) => {
        expect.step({
            ruleIds: args[0],
    ...
    onRpc("workflow.studio.approval.rule", "delete_approval", () => {
