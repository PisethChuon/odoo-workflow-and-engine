...
def uninstall_hook(env):
    ...
    _model_names = ['x_phone_allowance']
    _module_name = 'phone_allowance'  # Adjust this to your real module name
    ...
    try:
        cr.execute("""
            ...
            DROP COLUMN IF EXISTS x_request_item,
            DROP COLUMN IF EXISTS x_justification;
        """)
        # cr.execute("""
        #     DELETE FROM ir_model_fields 
        #     WHERE model = 'dynamic.import.wizard' 
        #     AND name IN ('x_request_item', 'x_justification');
        # """)
        cr.commit() 
